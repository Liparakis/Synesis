...
{
    throw 'Set SYNESIS_BOOTSTRAP_BASE_URL before running the installer.'
}
$name = "synesis-bootstrap-windows-$platform.exe"
...
try
{
    ...
    if (-not $line)
    {
        throw "No published checksum for $name"
    }
    $expected = ($line -split '\s+')[0].ToLowerInvariant()
    $actual = (Get-FileHash -Algorithm SHA256 -LiteralPath $file).Hash.ToLowerInvariant()
    if ($actual -ne $expected)
    {
        throw "Bootstrap checksum mismatch for $name"
    }
