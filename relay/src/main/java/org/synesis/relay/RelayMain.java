...
public final class RelayMain {
   ...
   *
   * @param arguments {@code --project UUID --node sl1-...} plus optional {@code --port},
   *                  {@code --identity-dir}, and {@code --workers}
   * @throws Exception if identity loading or binding fails
   */
  public static void main(String[] arguments) throws Exception {
    Arguments options = Arguments.parse(arguments);
    NodeIdentity identity = new IdentityBootstrap(options.identityDirectory()).loadOrCreate()
        .identity();
    ...
    OverlayRelayServer server = new OverlayRelayServer(
        new InetSocketAddress(options.bindHost(), options.port()), identity, relay,
        options.workers());
    server.start();
  ...
  private record Arguments(String bindHost, int port, int workers, UUID projectId,
