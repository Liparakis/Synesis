...
public final class OverlayRelayClient implements AutoCloseable {
   ...
   */
  public OverlayRelayClient(NodeIdentity identity, InetSocketAddress serverAddress,
      String expectedRelayNodeId, byte[] expectedRelayPublicKey,
      Consumer<OverlayForwardingFrame> inboundHandler) {
    this.identity = Objects.requireNonNull(identity, "identity");
    this.serverAddress = Objects.requireNonNull(serverAddress, "server address");
    this.expectedRelayNodeId = Objects.requireNonNull(expectedRelayNodeId,
        "expected relay node ID");
    this.expectedRelayPublicKey = Objects.requireNonNull(expectedRelayPublicKey,
        "expected relay public key").clone();
  ...
  public CompletionStage<OverlayRelayClient> connect() {
    ...
    try {
      ChannelFuture connection = new Bootstrap().group(group)
          .channel(NioSocketChannel.class).handler(new ChannelInitializer<SocketChannel>() {
            @Override
            protected void initChannel(SocketChannel channel) {
              channel.pipeline().addLast(
                  new LengthFieldBasedFrameDecoder(RelayWireCodec.MAX_PAYLOAD_BYTES + Integer.BYTES,
  ...
  public CompletionStage<Void> send(OverlayForwardingFrame frame) {
    ...
    if (!authenticated || closed || channel == null || !channel.isActive()) {
      return CompletableFuture.failedFuture(
          new IllegalStateException("relay client is not connected"));
    }
    CompletableFuture<Void> result = new CompletableFuture<>();
    if (pending.putIfAbsent(frame.messageId(), result) != null) {
      return CompletableFuture.failedFuture(
          new IllegalArgumentException("frame message is already pending"));
    }
    channel.writeAndFlush(Unpooled.wrappedBuffer(RelayWireCodec.encodeFrame(frame)))
        .addListener(future -> {
          ...
          if (result != null) {
            if (ack.status == 0) {
              ...
              result.completeExceptionally(
                  new IOException("relay rejected frame with status " + ack.status));
            }
          }
        } else {
