...
final class RelayWireCodec {
    ...
    static byte[] encodeHelloAck(NodeIdentity relayIdentity, Hello hello) throws GeneralSecurityException {
    ...
    static void verifyHelloAck(HelloAck ack, Hello hello, String expectedNodeId, byte[] expectedPublicKey)
    ...
    private static byte[] ackUnsigned(NodeIdentity relayIdentity, Hello hello) {
        return ackUnsigned(relayIdentity.nodeId(), relayIdentity.publicKeyEncoded(), hello.nodeId, hello.nonce);
    }
    ...
    private static byte[] ackUnsigned(String relayNodeId, byte[] publicKey, String clientNodeId, byte[] nonce) {
        try {
            ByteArrayOutputStream bytes = new ByteArrayOutputStream();
            try (DataOutputStream output = new DataOutputStream(bytes)) {
    ...
    private static void writeBytes(DataOutputStream output, byte[] value, int max) throws IOException {
