...
public final class ClaudeCodeProviderIntegration implements ProviderIntegration {
  ...
  /**
   * Builds a project-scoped MCP entry. The explicit project is a local configuration fallback for
   * Claude sessions that do not send MCP roots; it does not alter global provider configuration or
   * replace root-based resolution when roots are supplied.
   *
   ...
   */
  @Override
  public java.util.Optional<java.util.List<String>> autonomousCommand(Path worktree,
      String prompt) {
  ...
  /**
   * Returns the Claude Code hook adapter command using the canonical {@code claude} provider
   * identifier.
   *
