 ...
 * re-enumerates observable descendants after graceful and forced phases and never targets a process
 * whose root attachment identity cannot be proven. Detached or re-parented processes are reported
 * as unobservable rather than claimed clean. The default implementation is thread-safe for
 * independent calls; one invocation owns its deadline and process observations.
 *
 * @since 1.0
 */
...
public final class ProcessTreeTerminator {
  ...
  private static boolean mismatches(AttachmentIdentity expected, ObservedProcess actual) {
    return expected.pid() != actual.pid()
        || !expected.executable()
        .equals(actual.executable())
        || !expected.commandIdentity()
        .equals(actual.commandIdentity())
        || expected.startEpochMillis() != actual.startEpochMillis();
  }

  private static void waitUntil(long deadlineMillis) {
  ...
  public Result terminate(AttachmentIdentity identity, long expectedGeneration, Duration grace,
    ...
    ObservedProcess root = inspector.process(identity.pid())
        .orElse(null);
    if (root != null && mismatches(identity, root)) {
    ...
    long graceDeadline = Math.min(deadline.toEpochMilli(),
        System.currentTimeMillis() + grace.toMillis());
    while (System.currentTimeMillis() < graceDeadline) {
      Optional<ObservedProcess> refreshed = inspector.process(identity.pid());
      if (refreshed.isPresent() && mismatches(identity, refreshed.get())) {
        return result(Outcome.PROCESS_OWNERSHIP_UNPROVEN, identity, List.of(), false,
            "process_ownership_unproven");
      }
      ...
      List<ObservedProcess> descendants = ownedDescendants(root);
      lastObservedDescendants = descendants;
      descendants.forEach(inspector::graceful);
      if (rootAlive) {
        inspector.graceful(root);
      }
      waitUntil(graceDeadline);
      Optional<ObservedProcess> afterGrace = inspector.process(identity.pid());
      if (afterGrace.isPresent() && mismatches(identity, afterGrace.get())) {
        return result(Outcome.PROCESS_OWNERSHIP_UNPROVEN, identity, List.of(), false,
            "process_ownership_unproven");
      }
      if (afterGrace.isPresent()) {
        root = afterGrace.get();
      }
      rootPresent = afterGrace.isPresent();
      List<ObservedProcess> afterGraceDescendants = ownedDescendants(root);
      if ((!rootPresent || !inspector.alive(root)) && afterGraceDescendants.isEmpty()) {
        if (hasLiveUnobservableDescendant(lastObservedDescendants)) {
          return result(Outcome.DETACHED_OR_REPARENTED_UNOBSERVABLE, identity,
              lastObservedDescendants.stream()
                  .filter(inspector::alive)
                  .map(ObservedProcess::pid)
                  .toList(),
              lastObservedDescendants.stream()
                  .map(ObservedProcess::pid)
                  .toList(), false,
    ...
    while (System.currentTimeMillis() < deadline.toEpochMilli()) {
      ...
      waitUntil(deadline.toEpochMilli());
      Optional<ObservedProcess> afterForce = inspector.process(identity.pid());
      if (afterForce.isPresent() && mismatches(identity, afterForce.get())) {
        return result(Outcome.PROCESS_OWNERSHIP_UNPROVEN, identity, List.of(), forced,
            "process_ownership_unproven");
      }
      if (afterForce.isPresent()) {
        root = afterForce.get();
      }
      rootPresent = afterForce.isPresent();
      List<ObservedProcess> remaining = ownedDescendants(root);
    ...
    if (finalRoot.isPresent() && mismatches(identity, finalRoot.get())) {
      return result(Outcome.PROCESS_OWNERSHIP_UNPROVEN, identity, List.of(), forced,
          "process_ownership_unproven");
    }
    if (finalRoot.isPresent()) {
      root = finalRoot.get();
    }
    ...
    remaining.stream()
        .filter(inspector::alive)
        .map(ObservedProcess::pid)
        .forEach(survivors::add);
    if (rootPresent && inspector.alive(root)) {
      survivors.add(root.pid());
    }
    if (!rootPresent && remaining.isEmpty() && hasLiveUnobservableDescendant(
        lastObservedDescendants)) {
      return result(Outcome.DETACHED_OR_REPARENTED_UNOBSERVABLE, identity,
          lastObservedDescendants.stream()
              .filter(inspector::alive)
              .map(ObservedProcess::pid)
  ...
  private List<ObservedProcess> ownedDescendants(ObservedProcess root) {
    return inspector.descendants(root.pid())
        .stream()
        .filter(process -> process.pid() != root.pid())
        .filter(process -> process.startEpochMillis() > 0)
        .sorted(Comparator.comparingInt(ObservedProcess::depth)
            .reversed()
  ...
  public record Result(Outcome outcome, long attachmentGeneration, long rootPid,
                       List<Long> survivors, List<Long> observableDescendants, boolean forced,
                       String diagnostic) {
  ...
  private static final class JdkInspector implements Inspector {
    ...
    private static ObservedProcess observed(ProcessHandle handle, int depth) {
      ...
      String command = info.commandLine()
          .orElse(executable);
      long started = info.startInstant()
