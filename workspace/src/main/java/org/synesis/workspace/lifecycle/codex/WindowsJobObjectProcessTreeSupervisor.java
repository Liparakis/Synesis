 ...
 * kill-on-close Job, and resumes its primary thread only after assignment. The Java {@link Process}
 * returned to the lifecycle retains the native process and pipe handles; no later PID adoption or
 * descendant polling is used as the ownership boundary.</p>
 *
 * <p>This class intentionally exposes only the existing narrow supervisor
 * contract. It is Windows-only and requires Java native access to be enabled for the hosting
 * JVM.</p>
 *
 * @since 1.0
 */
@SuppressWarnings("restricted")
public final class WindowsJobObjectProcessTreeSupervisor implements ManagedProcessTreeSupervisor,
    AutoCloseable {
   ...
   *
   * @throws IOException when the current platform is not Windows or the native kernel API cannot be
   *                     loaded
   */
   ...
   */
  @Override
  public Process launch(List<String> command, Path workingDirectory,
  ...
  /**
   * Returns the native lifecycle state for a process retained by this supervisor.
   *
  ...
  /**
   * Terminates the complete Job and proves root termination plus zero active Job processes before
   * releasing all native handles.
   *
  ...
  private Handles createPipes(Arena arena) throws IOException {
    ...
    if (api.createPipe(inRead, inWrite, security) == 0
        || api.createPipe(outRead, outWrite, security) == 0
    ...
    return new Handles(inRead.get(ADDRESS, 0), outWrite.get(ADDRESS, 0), errWrite.get(ADDRESS, 0),
        parentIn,
        parentOut, parentErr);
  }
  ...
  private record Handles(MemorySegment childStdInput, MemorySegment childStdOutput,
                         MemorySegment childStdError,
                         MemorySegment parentStdInput, MemorySegment parentStdOutput,
                         MemorySegment parentStdError) {
  ...
  private static final class WindowsManagedProcess extends Process {
    ...
    private WindowsManagedProcess(NativeApi api, MemorySegment processHandle, long pid,
        MemorySegment stdin,
  ...
  private static final class NativeApi {
    ...
    private NativeApi() throws IOException {
      try {
        createJobObject = bind("CreateJobObjectW",
        ...
        createPipe = bind("CreatePipe", FunctionDescriptor.of(JAVA_INT, ADDRESS, ADDRESS, ADDRESS,
            JAVA_INT));
        createProcess = bind("CreateProcessW",
        ...
        terminateJobObject = bind("TerminateJobObject",
            FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT));
        terminateProcess = bind("TerminateProcess",
            FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT));
        waitForSingleObject = bind("WaitForSingleObject",
            FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT));
        ...
        isProcessInJob = bind("IsProcessInJob",
            FunctionDescriptor.of(JAVA_INT, ADDRESS, ADDRESS, ADDRESS));
        getProcessId = bind("GetProcessId", FunctionDescriptor.of(JAVA_INT, ADDRESS));
        getExitCodeProcess = bind("GetExitCodeProcess",
            FunctionDescriptor.of(JAVA_INT, ADDRESS, ADDRESS));
    ...
    private int setJobLimits(MemorySegment job, MemorySegment limits) throws IOException {
      return (int) invoke(setInformationJobObject, job, JOB_OBJECT_EXTENDED_LIMIT_INFORMATION,
          limits,
          (int) JOB_LIMITS_SIZE);
    }

    private int setHandleInherit(MemorySegment handle, boolean inherit) throws IOException {
      return (int) invoke(setHandleInformation, handle, HANDLE_FLAG_INHERIT,
          inherit ? HANDLE_FLAG_INHERIT : 0);
    }

    private int createPipe(MemorySegment read, MemorySegment write, MemorySegment security)
        throws IOException {
    ...
    private int createProcess(MemorySegment commandLine, int flags, MemorySegment environment,
        MemorySegment directory, MemorySegment startup, MemorySegment processInfo)
        throws IOException {
      return (int) invoke(createProcess, MemorySegment.NULL, commandLine, MemorySegment.NULL,
          MemorySegment.NULL,
    ...
    private int activeProcesses(MemorySegment job) throws IOException {
      try (Arena arena = Arena.ofConfined()) {
        ...
        int result = (int) invoke(queryInformationJobObject, job,
    ...
    private int writeFile(MemorySegment handle, MemorySegment buffer, int length,
