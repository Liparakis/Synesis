 ...
 * pending proof scope; the managed App Server creates and returns its own provider thread.
 * Successor preparation uses the exact durable owner. Raw proof remains in trusted process memory
 * until the child owns it, while the durable attachment record stores only the proof hash.</p>
 */
public final class ManagedCodexProcessLauncher implements
    CodexAppServerLifecycleService.ProcessLauncher {
   ...
   */
  public ManagedCodexProcessLauncher(ProjectApplicationService.ProjectLocation location,
      Path synesisLauncher) {
  ...
  public ManagedCodexProcessLauncher(ProjectApplicationService.ProjectLocation location,
    ...
    this(location, synesisLauncher, attachmentService,
        ManagedCodexRuntimeMode.NORMAL_PROVIDER_HOME_MANAGED);
  }
  ...
  public ManagedCodexProcessLauncher(ProjectApplicationService.ProjectLocation location,
    ...
    this(location, synesisLauncher, attachmentService, runtimeMode,
        ManagedProcessTreeSupervisor.platformDefault());
  }
  ...
  public ManagedCodexProcessLauncher(ProjectApplicationService.ProjectLocation location,
    ...
    this.processTreeSupervisor = Objects.requireNonNull(processTreeSupervisor,
        "processTreeSupervisor");
    this.deathReceiptStore = ManagedRuntimeDeathReceiptStore.storeFor(location);
  }

  private static List<String> configuredCommand(CodexManagedRuntimeHome home, Path launcher,
      Path projectRoot,
    ...
    command.add("-c");
    command.add(
        "mcp_servers.synesis.env_vars=[\"" + CodexManagedRuntimeHome.ATTACHMENT_PROOF_ENV + "\"]");
  ...
  /**
   * Prepares a first managed attachment before the managed App Server creates its provider thread.
   *
  ...
  public ManagedAttachmentRecord prepareFirst(
    ...
    try {
      ...
      ManagedAttachmentService.IssuedAttachment issued = attachmentService.issuePending(location,
          ...
          store);
      prepared.put(authority.bindingSessionId(),
          new PreparedLaunch(home, issued.proof(), issued.record()));
   ...
   * @throws Exception when trusted death evidence is missing or the old attachment is live,
   *                   ambiguous, terminal, or stale
   */
  public ManagedAttachmentRecord prepareReplacement(
      LifecycleControlRequestEnvelope.AuthorityContext authority,
    ...
    try {
      configureHome(home);
      ManagedAttachmentService.IssuedAttachment issued = attachmentService.replaceAfterTrustedDeath(
          location,
          authority.projectId(), authority.provider(), authority.bindingSessionId(), home.homeId(),
  ...
  public CodexAppServerLifecycleService.AppServerProcess launch(
    ...
    Process process = processTreeSupervisor.launch(command, Path.of(authority.realWorktree()),
        environment);
  ...
  /**
   * Verifies the provider result before managed authority can be considered joined to this
  ...
  public void finalizeManagedThread(LifecycleControlRequestEnvelope.AuthorityContext authority,
    ...
    try {
      ManagedAttachmentStore store = ManagedAttachmentService.storeFor(location,
          authority.bindingSessionId());
      ManagedAttachmentRecord bound = attachmentService.bindProviderThread(store,
          authority.provider(),
      ...
      prepared.put(authority.bindingSessionId(),
          new PreparedLaunch(launch.home(), launch.proof(), bound));
    ...
    try {
      ...
      if (launch != null && launch.record()
        ...
        prepared.put(authority.bindingSessionId(), new PreparedLaunch(launch.home(), launch.proof(),
            store.read()
                .orElseThrow(
                    () -> new IOException("managed_attachment_missing_after_activation"))));
      }
    } catch (Exception failure) {
  ...
  /**
   * Marks provider persistence only after the trusted App Server completion notification for the
   * active exact thread.
   *
   * @param authority            exact binding authority
   * @param attachmentGeneration managed generation
  ...
  public void providerTurnCompleted(LifecycleControlRequestEnvelope.AuthorityContext authority,
    ...
    try {
      ...
      if (record.generation() != attachmentGeneration
  ...
  /**
   * Tears down one exact managed process tree and only then marks its attachment disconnected.
   *
   * @param authority          exact binding authority
  ...
  public boolean teardownAndProveEmpty(LifecycleControlRequestEnvelope.AuthorityContext authority,
    ...
    CodexAppServerLifecycleService.AppServerProcess process = activeProcesses.get(
        authority.bindingSessionId());
    ...
    ManagedProcessTreeSupervisor.DeathEvidence evidence = processTreeSupervisor
        .teardownAndProveEmptyWithEvidence(process.process(), expectedGeneration,
            process.executable(),
  ...
  /**
   * Records a definitively empty process tree and removes the in-memory process handle from the
   * active generation.
   *
   * @param authority            exact binding authority
  ...
  /**
   * Terminalizes an attachment and releases its provider-thread tombstone only after terminal state
   * is durably recorded.
   *
   * @param authority exact binding authority
   * @throws Exception when terminalization or exact ownership release fails
   */
  public void terminalizeAndRelease(LifecycleControlRequestEnvelope.AuthorityContext authority)
      throws Exception {
    requireAuthority(authority);
    ManagedAttachmentService attachment = attachmentService;
    attachment.terminalize(
        ManagedAttachmentService.storeFor(location, authority.bindingSessionId()));
    ProviderThreadOwnershipRecord ownership = brokerFor(authority).ownership();
    ownershipStore.release(ownership.provider(), ownership.providerThreadId(),
        authority.bindingSessionId());
  }
  ...
  public ProviderThreadOwnershipRecord observeAndClaimProviderThread(
      ...
      throws IOException {
    requireAuthority(authority);
    return ownershipStore.acquire(authority.projectId(), authority.provider(),
        providerReturnedThreadId,
        authority.bindingSessionId());
  }

  private ManagedCodexThreadBroker brokerFor(
      LifecycleControlRequestEnvelope.AuthorityContext authority)
      throws IOException {
    return new ManagedCodexThreadBroker(
        ownershipStore.findByBinding("codex", authority.bindingSessionId())
            .orElseThrow(() -> new IOException("provider_thread_ownership_missing")));
  }

  private void configureHome(CodexManagedRuntimeHome home) throws IOException {
    CodexManagedAuthentication.Strategy authentication = CodexManagedAuthentication.forMode(
        runtimeMode,
  ...
  private record PreparedLaunch(CodexManagedRuntimeHome home, String proof,
                                ManagedAttachmentRecord record) {
