...
/**
 * Event-driven, project-local wake relay owned by the existing Codex lifecycle host.
 *
 * <p>The relay watches only the durable coordination event directory. It does
 * not poll a provider, inspect conversation content, choose tasks, or launch participants. Each
 * event causes a bounded projection scan; the admission service then targets only an exact active
 * participant with an exact dormant thread and explicit pending inbox item. The relay is an
 * owner-thread responsibility, uses a daemon thread so it cannot outlive that owner process, and is
 * closed with its lifecycle owner.</p>
 *
 * @since 1.0
 */
