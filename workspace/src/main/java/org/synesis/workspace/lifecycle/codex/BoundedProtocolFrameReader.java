 ...
 * materialized. The reader is thread-confined to its caller. An oversized frame retains only a
 * bounded diagnostic prefix and never grows its buffer.
 *
 * @since 1.0
 */
public final class BoundedProtocolFrameReader {
  ...
  private static int append(ByteArrayOutputStream payload, int count, int value,
      byte[] diagnosticPrefix,
      int diagnosticPrefixLength) throws OversizedFrameException {
    if (count >= MAX_FRAME_BYTES) {
      throw new OversizedFrameException(
          java.util.Arrays.copyOf(diagnosticPrefix, diagnosticPrefixLength),
          count + 1);
    }
    payload.write(value);
