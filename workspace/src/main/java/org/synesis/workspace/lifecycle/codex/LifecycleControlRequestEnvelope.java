...
public final class LifecycleControlRequestEnvelope {
    ...
    public record SignedEnvelope(LifecycleControlRequestEnvelope request, String signerNodeId,
        ...
        public boolean verify() throws GeneralSecurityException {
            if (!NodeIdentity.deriveNodeId(signerPublicKey).equals(signerNodeId)) {
                return false;
            }
        ...
        public byte[] encoded() {
            ...
            value.put("signerPublicKey", Base64.getEncoder().encodeToString(signerPublicKey));
            value.put("signature", Base64.getEncoder().encodeToString(signature));
            byte[] encoded = ProviderJson.write(value).getBytes(StandardCharsets.UTF_8);
        ...
        public static SignedEnvelope decode(byte[] bytes) throws IOException {
            ...
            try {
                ...
                byte[] key = Base64.getDecoder().decode(string(value, "signerPublicKey"));
                byte[] signature = Base64.getDecoder().decode(string(value, "signature"));
    ...
    private static LifecycleControlRequestEnvelope fromMap(Map<String, Object> value) {
        ...
        return new LifecycleControlRequestEnvelope(
                ...
                new AuthorityContext(string(authority, "projectId"), string(authority, "controlProjectRoot"),
                        ...
                        integer(authority, "bindingVersion"),
