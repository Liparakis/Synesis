 ...
 * Transport retries must reuse the same request object and absolute deadline; changing any field
 * changes the digest and is a different logical request. The record is immutable and thread-safe.
 * It contains identifiers and bounded user input, but callers should still avoid logging the
 * complete payload.
 *
 * @since 1.0
 */
...
public final class LifecycleControlRequestEnvelope {
   ...
   */
  public LifecycleControlRequestEnvelope(UUID requestId, String hostInstanceId,
      AuthorityContext authority,
      Operation operation, long expectedLifecycleRevision, String expectedThreadId,
      String expectedTurnId,
      boolean continuation, String input, long callerDeadlineEpochMillis,
  ...
  private static LifecycleControlRequestEnvelope fromMap(Map<String, Object> value) {
    ...
    return new LifecycleControlRequestEnvelope(
        ...
        new AuthorityContext(string(authority, "projectId"),
            string(authority, "controlProjectRoot"),
  ...
  public SignedEnvelope sign(NodeIdentity signer) throws GeneralSecurityException {
    ...
    return new SignedEnvelope(this, signer.nodeId(), signer.publicKeyEncoded(),
        signer.sign(canonicalBytes()));
  }
  ...
  public record AuthorityContext(
    ...
    /**
     * Compatibility constructor for direct service fixtures that do not need a control-root
     * override.
     *
     * @param projectId            project UUID text
    ...
    public AuthorityContext(String projectId, String provider, String connectionInstanceId,
        ...
        String gitCommonDirectory, String branch, String baseCommit, String supervisorId,
        String workerId) {
  ...
  public record SignedEnvelope(LifecycleControlRequestEnvelope request, String signerNodeId,
    ...
    public byte[] encoded() {
      Map<String, Object> value = new LinkedHashMap<>();
      value.put("request",
          ProviderJson.parse(new String(request.canonicalBytes(), StandardCharsets.UTF_8)));
