 ...
 * JSON parsing or signature verification. WAIT is delegated to the host, which registers its waiter
 * under the lifecycle lock and waits outside it with bounded per-binding and per-host capacity. A
 * disconnected client cannot block protocol reading or lifecycle transitions. The adapter is
 * thread-safe.
 *
 * @since 1.0
 */
public final class CodexLifecycleHttpAdapter implements HttpHandler {
  ...
  private static void writeResponse(java.io.OutputStream output,
    ...
    if (encoded.length > CodexLifecycleHttpClient.MAX_RESPONSE_BYTES) {
      encoded = new CodexLifecycleHttpClient.Response(false, "lifecycle_response_overflow",
          "FAILED", 0L,
  ...
  private static int diagnosticStatus(Exception failure) {
    ...
    return message.contains("oversized") ? 413
        : message.contains("signature") || message.contains("owner")
          ? 401 : message.contains("conflict") ? 409 : 400;
  }

  private static String diagnostic(Exception failure) {
    String message = failure.getMessage() == null ? failure.getClass()
  ...
  public void handle(HttpExchange exchange) throws IOException {
    try (exchange) {
      ...
      try {
        ...
        if (signed.request()
            .operation() == LifecycleControlRequestEnvelope.Operation.WAIT) {
