...
public final class ProjectCommandDiagnostics {
  ...
  public static Report inspect(Path namespaceRoot) {
    ...
    Map<String, Object> indexValue = readObject(index, ProjectCommandNamespace.MAX_INDEX_BYTES,
        counters);
    if (indexValue == null) {
    ...
    counters.liveAtCapacityCount = counters.recordCounts.entrySet()
        .stream()
        .filter(
            entry -> entry.getValue() >= 8_192 && !counters.deadAnchorIds.contains(entry.getKey()))
        .mapToInt(ignored -> 1)
        .sum();

    boolean staleIndex = indexValue != null
        && number(indexValue, "scopeCount") != counters.scopeCount;
    return new Report(true, counters.formatValid, counters.newerObjectCount,
        ...
        staleIndex ? 1 : 0, counters.temporaryArtifactCount,
   ...
   */
  public record Report(boolean present, boolean formatValid, int newerObjectCount,
                       int olderFormatCount,
                       int corruptObjectCount, int permanentLockCount, int scopeCount,
                       int anchorCount,
                       int requestCount,
                       int liveAtCapacityCount, int deadAnchorCount, int eligibleTerminalCount,
                       int pinnedEvidenceCount,
                       int staleIndexCount, int temporaryArtifactCount,
                       int terminalHistoryCompactionCount,
                       int leaseGapRevisionMismatchCount,
                       int admissionRestartCount, int cleanCloseDetachBlockedCount,
                       int deferredMutationCount,
