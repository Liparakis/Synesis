 ...
 * event enters a subscriber queue. A slow subscriber is marked for refresh instead of causing the
 * publisher to block or retaining unbounded state.
 */
public final class ControlPlaneEventHub {
  ...
  private static String safeValue(OnboardingEvent event) {
    ...
    return switch (type) {
      case CANDIDATES_GATHERED, PATH_SELECTED, WORK_RESULT, NODE_ID, PEER_IDENTITY_VERIFIED,
           LIVENESS -> event.value();
      default -> "";
    };
  }
  ...
  public Subscription subscribe() {
    Subscription subscription = new Subscription(new ArrayBlockingQueue<>(QUEUE_CAPACITY),
        subscribers);
  ...
  public static final class Subscription implements AutoCloseable {
    ...
    private Subscription(ArrayBlockingQueue<Event> queue,
