...
public final class HookApplicationService {
  ...
  private static String evidence(String json) {
    ...
    for (String key : new String[]{"session_id", "sessionId", "conversation_id",
        "conversationId"}) {
  ...
  private static Path workspacePath(String json, Path fallback) {
    ...
    if (paths instanceof java.util.List<?> list && !list.isEmpty()
        && list.getFirst() instanceof String path
        && !path.isBlank()) {
      return Path.of(path);
    }
    return fallback;
    ...
    while (current != null) {
      ...
      if (java.nio.file.Files.isRegularFile(marker)) {
        Map<String, Object> value = object(
            ProviderJson.parse(java.nio.file.Files.readString(marker)));
  ...
  public HookExecutionResult codex(InputStream input) {
    try {
      ...
      ProviderSessionBindingService.BindingResult binding = bindings.findByWorktree(location,
              "codex", eventCwd)
