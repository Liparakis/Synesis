package org.synesis.workspace.application.sync;
...
public final class SyncApplicationService {
  ...
  private static boolean isReconciliation(byte[] bytes) {
    return bytes != null && bytes.length >= 4 && bytes[0] == 0x50 && bytes[1] == 0x52
        && bytes[2] == 0x50
        && bytes[3] == 0x31;
  }

  private static String composeInvitation(String link, UUID project, UUID record, String host) {
    return link + "?project=" + project + (record == null ? "" : "&record=" + record) + "&host="
        + host;
  }
  ...
  public SyncResult host(Path profile, String project, String record,
      Consumer<String> invitationSink) {
    ...
    try {
      ...
      Onboarding onboarding = new Onboarding(profile.resolve("link"), event -> {
        if (event.type() == OnboardingEventType.SHARE_LINK) {
          invitationSink.accept(
              composeInvitation(event.value(), config.projectId(), recordId, hostNode));
        }
      });
      try {
   ...
   * @return structured result
   */
  public SyncResult join(Path profile, String project, String record, String expectedHost,
      String invitation) {
    Objects.requireNonNull(profile, "profile");
    try {
      URI uri;
      ...
      try {
        ...
        clean = new URI(uri.getScheme(), uri.getAuthority(), uri.getPath(), null,
      ...
      UUID projectId = parseUuid(project != null ? project : query.get("project"),
          "PROJECT_INVALID");
      if (projectId == null) {
        throw failure("USAGE");
      }
