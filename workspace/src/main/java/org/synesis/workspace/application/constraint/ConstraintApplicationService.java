...
public final class ConstraintApplicationService {
   ...
   */
  public ConstraintCreateResult create(ProjectApplicationService.ProjectLocation location,
      String title,
    ...
    try {
      ...
      NodeIdentity identity = new IdentityBootstrap(location.profile()
          ...
          .identity();
      DecisionRecord record = ProjectConstraint.createTypedRecord(config.projectId(),
          UUID.randomUUID(),
      ...
      if (saved != DecisionStore.SaveResult.APPLIED
          && saved != DecisionStore.SaveResult.DUPLICATE) {
