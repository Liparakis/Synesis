...
/**
 * Provider-neutral seam for authenticating a runtime attachment to an existing logical provider
 * binding.
 *
 * <p>Implementations return only an already-authorized binding reference and
 * its current attachment generation. They do not create participants, WorkIntents, claims, or
 * workgroups.</p>
 */
@FunctionalInterface
public interface RuntimeAuthenticator {
   ...
   * @param threadId           exact provider thread, or {@code null} for a pending first
   *                           generation
