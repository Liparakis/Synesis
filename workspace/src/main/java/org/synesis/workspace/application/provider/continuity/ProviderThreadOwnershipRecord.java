 ...
 * Its key is the provider and exact provider thread; attachment generations remain a separate
 * runtime-freshness fence.</p>
 *
...
public record ProviderThreadOwnershipRecord(
  ...
  public ProviderThreadOwnershipRecord {
    ...
    if (revision < 1 || acquiredAtEpochMillis <= 0 || updatedAtEpochMillis <= 0) {
      throw new IllegalArgumentException(
          "invalid provider-thread ownership timestamps or revision");
    }
  }
  ...
  public ProviderThreadOwnershipRecord(int schemaVersion, String projectId, String provider,
    ...
    this(schemaVersion, projectId, provider, providerThreadId, bindingSessionId, status, false,
        revision,
  ...
  private static void requireText(String value, String label) {
    ...
    if (value.isBlank() || value.length() > 8_192 || value.indexOf('\r') >= 0
        || value.indexOf('\n') >= 0) {
