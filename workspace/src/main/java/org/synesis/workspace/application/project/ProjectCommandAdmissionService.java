...
public final class ProjectCommandAdmissionService {
  ...
  public AgentResponse execute(ProjectCommandService.CommandRequest request, Object typedRequestId,
    ...
    try {
      Optional<ProjectCommandRecord> existing = store.find(worktree.locator(), anchor.anchorId(),
          requestId);
    ...
    try (ProjectCommandProtectionService.ProtectionPermit permit = protectionService.acquire(
        worktree)) {
      if (!permit.isHeld()) {
    ...
    try (ProjectCommandProtectionService.ProtectionPermit permit = protectionService.acquire(
      ...
      Optional<ProjectCommandRecord> raced = store.find(worktree.locator(), anchor.anchorId(),
          requestId);
      if (raced.isPresent()) {
        return blocked(AgentReason.COMMAND_AMBIGUOUS, "REQUEST_APPEARED_DURING_ADMISSION_GAP");
      }
      if (store.countForAnchor(worktree.locator(), anchor.anchorId())
          >= MAX_REQUEST_IDS_PER_LIVE_ANCHOR) {
    ...
    try {
      AgentResponse response = commandService.runCommand(request, started -> {
        try (ProjectCommandProtectionService.ProtectionPermit permit = protectionService.acquire(
          ...
          ProjectCommandRecord running = new ProjectCommandRecord(
              ...
              "commandLine", started.commandLine(), "processStartTime",
      ...
      ProjectCommandRecord terminal = new ProjectCommandRecord(
          ...
          stdoutComplete, stderrComplete, null, revision.incrementAndGet(),
          starting.createdAtEpochMillis(),
