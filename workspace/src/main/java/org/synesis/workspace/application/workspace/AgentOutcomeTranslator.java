...
public final class AgentOutcomeTranslator {
  ...
  public TranslatedOutcome translateMutationResult(MutationResult result, String relativePath) {
    ...
    return switch (decision) {
      ...
      case DENY_POLICY -> {
        ...
        AgentReason reason =
            isProtected ? AgentReason.PROTECTED_CONFIGURATION : AgentReason.POLICY_DENIED;
      ...
      default ->
          translateException(new IllegalArgumentException("Unknown broker decision: " + decision));
    };
  }
  ...
  public TranslatedOutcome translatePendingOwner() {
    ...
    return new TranslatedOutcome(response, null, "OWNER_RESPONSE_PENDING", null, null, false, true,
        false);
  }
  ...
  /**
   * Translates an internal exception into a safe failure response without leaking raw exception
   * details.
   *
