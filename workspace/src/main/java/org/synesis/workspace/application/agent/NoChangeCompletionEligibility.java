...
final class NoChangeCompletionEligibility {
    ...
    static Result assess(PredictionEventStore store, WorkIntent intent, String participant,
        ...
        for (LaneGrant grant : store.workGroupProjection()
            ...
            boolean targetsThisIntent = grant.targetIntentId()
                    .equals(intent.intentId())
                    && grant.claimEpoch() == intent.version();
            boolean targetsCaller = grant.targetParticipant()
                    .equals(participant);
        ...
        for (CoordinationRequest request : store.collaborationProjection()
            ...
            if (request.conflictingIntentId()
                    .equals(intent.intentId())
                    || request.requester()
                    .equals(participant)
                    || request.target()
        ...
        if (hasPendingCapability(store, nodeId, supervisorId, workerId)
                || !store.capabilityRequestProjection()
