 ...
 * workflow. It may establish the binding, participant, WorkIntent, acquired claim, lane, epoch, and
 * worktree, but it cannot launch Codex or write the lifecycle idempotency ledger during that phase.
 * Only the resulting immutable authority context is submitted to the lifecycle owner. The class is
...
public final class ProviderSessionCommand {
  ...
  public ProviderSessionCommand() {
    this(new AgentSessionService(), new WorkspaceCollaborationService(),
        new ProviderSessionBindingService());
  }
  ...
  public ProviderSessionCommand(AgentSessionService sessionService,
      WorkspaceCollaborationService collaborationService,
      ProviderSessionBindingService bindingService) {
    this.sessionService = Objects.requireNonNull(sessionService, "sessionService");
    this.collaborationService = Objects.requireNonNull(collaborationService,
  ...
  public CodexLifecycleHttpClient.Response start(StartRequest request) throws Exception {
    ...
    if (!binding.sessionId()
        .equals(session.sessionId()) || !"VERIFIED".equals(binding.verificationState())
    ...
    LifecycleControlRequestEnvelope.AuthorityContext authority =
        new LifecycleControlRequestEnvelope.AuthorityContext(
            ...
            binding.workerId());
    LifecycleControlRequestEnvelope envelope = new LifecycleControlRequestEnvelope(
        UUID.randomUUID(),
  ...
  /**
   * Signs a previously established Codex lifecycle envelope with the local project identity.  The
   * caller can retain the returned immutable value for byte-equivalent transport retries.
   *
  ...
  public LifecycleControlRequestEnvelope.SignedEnvelope sign(Path projectRoot,
    ...
    ProjectApplicationService.ProjectLocation location = new ProjectApplicationService().locate(
        projectRoot.toAbsolutePath()
            .normalize());
    NodeIdentity signer = new IdentityBootstrap(location.profile()
        .resolve("link")).loadOrCreate()
        .identity();
  ...
  /**
   * Submits one already signed Codex lifecycle envelope through the production owner's loopback
   * route.  The operation encoded in the envelope determines whether the owner invokes START,
   * NOTIFY, STEER, WAIT, INTERRUPT, HARD_STOP, RESUME, or STATUS.
   *
  ...
  /**
   * Signs and submits one immutable Codex lifecycle operation.  This is the production call path
   * for non-START operations after the caller has resolved the exact authority context through the
   * existing Synesis session workflow.
   *
   * @param projectRoot initialized control project root
   * @param endpoint    existing coordination owner endpoint
  ...
  public record StartRequest(Path projectRoot, URI endpoint, String hostInstanceId,
                             String connectionInstanceId, String goal, String acceptance,
                             List<ResourceSelector> selectors,
                             String input, long expectedLifecycleRevision,
