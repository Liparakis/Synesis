 ...
 * process streams concurrently, retains bounded head/tail evidence, counts all raw bytes read, and
 * kills the complete process tree on timeout, cancellation, or termination. It is thread-safe
 * because each invocation owns its process and collectors.</p>
 *
 * @since 1.0
 */
public final class ProjectProcessExecutor {
  ...
  private static Outcome classifyStartFailure(IOException failure) {
    ...
    if (message.contains("denied") || message.contains("permission") || message.contains(
        "access is denied")) {
  ...
  public ExecutionResult execute(ExecutionRequest request,
      Consumer<StartedProcessIdentity> startedObserver) {
    ...
    try {
      ...
      Thread stdoutThread = startCollector(process.getInputStream(), stdout,
          "synesis-command-stdout");
      Thread stderrThread = startCollector(process.getErrorStream(), stderr,
          "synesis-command-stderr");
      boolean finished;
      try {
  ...
  /**
   * One execution request. The working directory is resolved relative to {@link #worktreeRoot()}
   * and never relative to the server process.
   *
  ...
  public record ExecutionRequest(List<String> argv, Path worktreeRoot, String workingDirectory,
     ...
     */
    public static ExecutionRequest from(ProjectCommandSpec spec, Path worktreeRoot,
        Path controlRoot) {
  ...
  public record ExecutionResult(Outcome outcome, Integer exitCode, String stdout, String stderr,
                                long stdoutBytesRead, long stderrBytesRead,
                                long stdoutBytesRetained,
  ...
  private static final class StreamCollector {
    ...
    private Evidence evidence() {
      ...
      if (!overflow && eof) {
        ...
        return new Evidence(new String(complete, StandardCharsets.UTF_8), bytesRead, bytesRead,
            false);
      }
      ...
      boolean boundaryDiscarded =
          safeHead.length != headBytes.length || safeTail.length != tailBytes.length;
