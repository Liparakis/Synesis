...
public final class CapabilityResponseService {
  ...
  public AgentResponse respondToOwnerRequest(OwnerResponseRequest request) {
    ...
    try {
      ...
      String respType = request.response()
          .trim()
      ...
      return switch (respType) {
        case "accept" -> {
          CapabilityRequestPayload payload = new CapabilityRequestPayload(
              record.handle(),
              record.capability(),
              record.requesterNodeId(),
              record.requesterSupervisorId(),
              record.requesterWorkerId(),
              ownerNodeId,
        ...
        case "revise" -> {
          CapabilityContract revContract =
              request.revision() != null ? request.revision() : record.contract();
          CapabilityRequestPayload payload = new CapabilityRequestPayload(
              record.handle(),
              record.capability(),
              record.requesterNodeId(),
        ...
        case "reject" -> {
          String reason = request.reason() != null && !request.reason()
              .isBlank() ? request.reason() : "Capability request rejected by owner";
          CapabilityRequestPayload payload = new CapabilityRequestPayload(
              record.handle(),
              record.capability(),
              record.requesterNodeId(),
              ...
              reason);
          store.append(UUID.randomUUID(),
              PredictionEventType.CAPABILITY_REQUEST_REJECTED,
              ownerNodeId,
              payload.encode(),
              identity);
