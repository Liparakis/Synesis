...
public final class IntegrationCompatibilityService {
    ...
    public enum FailureCode {
        /** Snapshot base is not the expected control head. */ STALE_BASE,
        /** Two snapshots modify the same normalized path. */ OVERLAPPING_SNAPSHOT,
        /** A changed path is outside the snapshot's declared claims. */ UNCOVERED_PATH,
        /** A dependency references a non-current contract revision. */ STALE_CONTRACT,
        /** A direct filesystem mutation is outside declared ownership. */ OUT_OF_BAND_MUTATION,
        /** Configured project tests failed. */ TESTS_FAILED
    }
    ...
    public record SnapshotInput(String snapshotId, String baseCommit, List<String> changedPaths,
            List<ResourceSelector> claims, List<ContractReference> contracts, List<String> outOfBandPaths) {
    ...
    public CheckResult check(CheckRequest request) {
        ...
        for (SnapshotInput snapshot : request.snapshots()) {
            if (!request.controlHead().equals(snapshot.baseCommit())) {
            ...
            for (String path : snapshot.changedPaths()) {
                ...
                boolean covered = snapshot.claims().stream().anyMatch(selector -> selector.overlaps(ResourceSelector.pathExact(normalized)));
                if (!covered) {
                    failures.add(FailureCode.UNCOVERED_PATH);
                    actions.add("Claim changed path before publishing " + normalized);
                }
            }
            if (!snapshot.outOfBandPaths().isEmpty()) {
            ...
            for (ContractReference dependency : snapshot.contracts()) {
                boolean current = request.currentContracts().stream().anyMatch(contract ->
                        contract.contractId().equals(dependency.contractId())
