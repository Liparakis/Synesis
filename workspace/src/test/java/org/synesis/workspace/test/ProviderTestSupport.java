...
/**
 * Installs real project provider integrations for tests that exercise agent admission or workspace
 * readiness.
 */
public final class ProviderTestSupport {
  ...
  /**
   * Installs one provider while isolating provider-global configuration from the developer's real
   * home directory.
   *
   ...
   */
  public static void install(ProjectApplicationService.ProjectLocation location, String provider)
      throws Exception {
    isolateHome();
    ProviderApplicationService.ProviderResult result = new ProviderApplicationService().install(
        location,
        provider);
    if (result.exitCode() != 0 || !"BOUND".equals(result.values()
        .get("SESSION_BINDING"))) {
      throw new IllegalStateException(
          "Provider test installation did not bind a session: " + result.values());
    }
  }
