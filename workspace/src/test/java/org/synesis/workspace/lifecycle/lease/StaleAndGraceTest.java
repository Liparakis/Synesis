...
class StaleAndGraceTest {
  ...
  void evaluatesSuspectedStaleAndRecoveryEligibleStates() {
    ...
    SessionProcessIdentity identity = new SessionProcessIdentity(99999L,
        "java",
        "java -jar synesis.jar",
        start,
        "nonce123");
    ...
    Clock clockStale = Clock.fixed(Instant.ofEpochMilli(start + 180000L),
        ZoneId.of("UTC")); // 3 min later
    SessionLeasePolicy policyStale = new SessionLeasePolicy(clockStale,
    ...
    Clock clockGrace = Clock.fixed(Instant.ofEpochMilli(start + 360000L),
        ZoneId.of("UTC")); // 6 min later
    SessionLeasePolicy policyGrace = new SessionLeasePolicy(clockGrace,
        Duration.ofSeconds(30),
        Duration.ofMinutes(2),
        Duration.ofMinutes(5));
