...
public final class RecordMessage {
            ...
            try (DataOutputStream output = new DataOutputStream(bytes)) {
                ...
                switch (kind) {
                    case SYNC_REQUEST -> {
                        ...
                        if (knownDigest != null) output.write(knownDigest);
                    }
                    ...
                    case RESULT -> {
                        ...
                        if (digest != null) output.write(digest);
                    }
                    case ERROR -> {
    ...
    private static RecordMessage decodeRequest(DataInputStream input) throws IOException {
        ...
        if (marker > 1 || (marker == 1 && input.available() < 32)) throw new IOException("invalid request head");
    ...
    private static RecordMessage decodeRecord(DataInputStream input) throws IOException {
        ...
        if (input.available() != 0) throw new IOException("trailing record message bytes");
        return record(bytes);
    }

    private static RecordMessage decodeResult(DataInputStream input) throws IOException {
        int code = input.readUnsignedByte();
        if (code >= ResultCode.values().length) throw new IOException("unknown result");
        UUID record = readUuid(input);
        long revision = input.readLong();
        int marker = input.readUnsignedByte();
    ...
    private static RecordMessage decodeError(DataInputStream input) throws IOException {
        int code = input.readUnsignedByte();
        if (code >= ErrorCode.values().length) throw new IOException("unknown error");
        String text = readText(input, MAX_ERROR_TEXT_BYTES);
        if (input.available() != 0) throw new IOException("trailing error bytes");
    ...
    private static byte[] readBytes(DataInputStream input) throws IOException {
        ...
        if (length == 0 || length > input.available()) throw new IOException("invalid message bytes");
