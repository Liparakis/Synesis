package org.synesis.projectrecord.domain;
...
public final class DecisionRecord {
  ...
  private DecisionRecord(UUID projectId, UUID recordId, RecordType recordType, long revision,
    ...
    if (recordType == RecordType.PROJECT_CONSTRAINT) {
      this.constraintPayload = Objects.requireNonNull(constraintPayload,
          "constraint payload required");
    } else {
      if (constraintPayload != null) {
        throw new IllegalArgumentException(
            "non-constraint record must not carry constraint payload");
      }
   ...
   */
  public static DecisionRecord create(UUID projectId, UUID recordId, long revision,
      byte[] previousDigest,
    ...
    return createTyped(projectId, recordId, RecordType.DECISION, revision, previousDigest,
        ownerNodeId,
   ...
   * @throws GeneralSecurityException if signing fails
   */
  public static DecisionRecord createConstraint(UUID projectId, UUID recordId, long revision,
      byte[] previousDigest,
      String ownerNodeId, String authorNodeId, DecisionStatus status, Instant createdAt,
      Instant updatedAt, String title, String rationale, List<DecisionEvidence> evidence,
      ConstraintPayload constraintPayload, Ed25519Signer signer) throws GeneralSecurityException {
    return createTyped(projectId, recordId, RecordType.PROJECT_CONSTRAINT, revision, previousDigest,
        ownerNodeId,
        authorNodeId, status, createdAt, updatedAt, title, rationale, evidence, constraintPayload,
        signer);
  }
  ...
  private static DecisionRecord createTyped(UUID projectId,
    ...
    DecisionRecord unsigned = new DecisionRecord(projectId, recordId, recordType, revision,
        previousDigest,
  ...
  public static DecisionRecord decode(byte[] bytes) throws IOException {
    ...
    try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      return new DecisionRecord(projectId, recordId, recordType, revision, previousDigest, owner,
          author,
  ...
  private byte[] encodeComplete() {
    try {
      ByteArrayOutputStream bytes = new ByteArrayOutputStream(
          unsignedBytes.length + signature.length + 2);
