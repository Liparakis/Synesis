...
public final class ProjectConstraint {
  ...
  public static DecisionRecord createTypedRecord(UUID projectId, UUID recordId, String ownerNode,
    ...
    return createTypedRecord(projectId, recordId, ownerNode, effect, scope, title, rationale,
        List.of(), signer);
  }

  /**
   * Creates a new signed {@link DecisionRecord} representing a typed project constraint with
   * supersedes.
   *
   * @param projectId  namespace project ID
   * @param recordId   record identity
  ...
  /**
   * Filters a list of active constraints to exclude any constraint that is explicitly superseded by
   * another active constraint in the list via its {@link #supersedes()} record IDs.
   *
  ...
  /**
   * Evaluates whether this active constraint applies to a normalized target path using
   * {@link ScopeMatcher}.
   *
