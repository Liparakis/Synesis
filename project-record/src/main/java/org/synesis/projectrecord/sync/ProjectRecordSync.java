...
public final class ProjectRecordSync {
    ...
    private CompletionStage<byte[]> handle(String remoteNodeId, byte[] payload) {
        ...
        try {
            if (!config.allows(remoteNodeId)) {
                result.complete(RecordMessage.error(RecordMessage.ErrorCode.UNAUTHORIZED, "peer not allowed")
                        .encoded());
                return result;
            }
            ...
            result.complete(RecordMessage.error(RecordMessage.ErrorCode.STORAGE_FAILURE, "storage failure")
                    .encoded());
        }
        return result;
    }
    ...
    private SyncOutcome await(CompletionStage<byte[]> stage, UUID recordId, String remoteNodeId) {
        try {
            ...
            return new SyncOutcome(response.resultCode() == null ? Code.REJECTED : Code.valueOf(response.resultCode()
                                                                                                .name()),
