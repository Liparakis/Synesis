...
import org.synesis.projectrecord.sync.protocol.RecordMessage;
...
public final class ProjectRecordSync {
  ...
  private static SyncOutcome rejectedOutcome(DecisionRecord record, String detail) {
    return new SyncOutcome(Code.REJECTED, record.recordId(), record.revision(), record.digest(),
        detail);
  }
  ...
  public SyncOutcome publish(PeerSession session, DecisionRecord record) {
    ...
    if (!config.projectId()
        .equals(record.projectId()) || !session.localNodeId()
    ...
    try {
      ...
      CompletionStage<byte[]> response = session.requestApplication(
          RecordMessage.record(record.encoded())
              .encoded());
      return await(response, record.recordId(), session.remoteNodeId());
    } catch (GeneralSecurityException failure) {
      return rejectedOutcome(record, "invalid signature");
    } catch (Exception failure) {
      return new SyncOutcome(Code.UNKNOWN, record.recordId(), 0, null,
  ...
  public SyncOutcome sync(PeerSession session, UUID recordId) {
    ...
    try {
      DecisionStore.Head head = store.headState(recordId)
          .orElse(null);
  ...
  private CompletionStage<byte[]> handle(String remoteNodeId, byte[] payload) {
    ...
    try {
      if (!config.allows(remoteNodeId)) {
        result.complete(
            RecordMessage.error(RecordMessage.ErrorCode.UNAUTHORIZED, "peer not allowed")
                .encoded());
        return result;
      }
      ...
      result.complete(RecordMessage.error(RecordMessage.ErrorCode.MALFORMED, "malformed message")
          .encoded());
    } catch (RuntimeException failure) {
      result.complete(
          RecordMessage.error(RecordMessage.ErrorCode.STORAGE_FAILURE, "storage failure")
              .encoded());
    }
    return result;
  }
  ...
  private RecordMessage handleRequest(RecordMessage request) throws IOException {
    ...
    DecisionRecord current = store.head(request.recordId())
        .orElse(null);
    if (current == null) {
      return RecordMessage.error(RecordMessage.ErrorCode.NOT_FOUND, "record not found");
    }
    if (request.knownRevision() == current.revision()
        && Arrays.equals(request.knownDigest(), current.digest())) {
      return RecordMessage.result(RecordMessage.ResultCode.DUPLICATE,
          current.recordId(),
          current.revision(),
          current.digest());
    }
    if (current.revision() > request.knownRevision()) {
      return RecordMessage.record(current.encoded());
    }
    if (current.revision() == request.knownRevision()) {
      return RecordMessage.result(RecordMessage.ResultCode.CONFLICT,
          current.recordId(),
          current.revision(),
          current.digest());
    }
    return RecordMessage.result(RecordMessage.ResultCode.REMOTE_STALE,
        current.recordId(),
        current.revision(),
        current.digest());
  }

  private RecordMessage handleRecord(String remoteNodeId, RecordMessage message)
      throws IOException {
    ...
    DecisionStore.Head head = store.headState(record.recordId())
        .orElse(null);
    return RecordMessage.result(mapResult(applied), record.recordId(),
        head == null ? 0 : head.revision(),
        head == null ? null : head.digest());
  }

  private SyncOutcome await(CompletionStage<byte[]> stage, UUID recordId, String remoteNodeId) {
    try {
      RecordMessage response = RecordMessage.decode(stage.toCompletableFuture()
          .get());
      ...
      return new SyncOutcome(
          response.resultCode() == null ? Code.REJECTED : Code.valueOf(response.resultCode()
                                                                       .name()),
