package org.synesis.projectrecord.guardrail;
 ...
 * backslashes are normalized. Absolute paths and directory traversal ('..') are rejected as invalid
 * input.
 *
 * @since 1.0
 */
public final class ScopeMatcher {
  ...
  public static String normalizePath(String path) {
    ...
    if (unixPath.length() >= 2 && Character.isLetter(unixPath.charAt(0))
        && unixPath.charAt(1) == ':') {
      throw new IllegalArgumentException(
          "Absolute paths with drive letters are not supported: " + path);
    }
  ...
  private static String normalizePattern(String pattern) {
    ...
    if (unixPattern.length() >= 2 && Character.isLetter(unixPattern.charAt(0))
        && unixPattern.charAt(1) == ':') {
    ...
    for (String seg : segments) {
      if ("..".equals(seg)) {
        throw new IllegalArgumentException(
            "Directory traversal '..' in pattern is forbidden: " + pattern);
      }
    }
