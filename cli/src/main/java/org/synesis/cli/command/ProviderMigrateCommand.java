...
public final class ProviderMigrateCommand implements Callable<Integer> {
    ...
    @Option(names = "--dry-run", description = "Inspect provider migration state without changing files.") private boolean dryRun;
    @Option(names = "--prepare", description = "Create an immutable provider migration plan.") private boolean prepare;
     ...
     */
    public ProviderMigrateCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            if (prepare) {
                ...
                runtime.terminal().stdout("PROVIDER_MIGRATION_RESULT=PLAN_PREPARED");
                runtime.terminal().stdout("PLAN_ID=" + plan.planId());
                return 0;
            }
            if (showPlan != null) {
                var plan = service.load(showPlan);
                runtime.terminal().stdout("PROVIDER_MIGRATION_RESULT=PLAN");
                runtime.terminal().stdout("PLAN_ID=" + plan.planId());
                runtime.terminal().stdout("PROVIDERS_INSPECTED=" + plan.entries().size());
                return 0;
            }
            if (execute != null) {
                var result = service.execute(service.load(execute));
                runtime.terminal().stdout("PROVIDER_MIGRATION_RESULT=" + result.outcome());
                runtime.terminal().stdout("BACKUPS_CREATED=" + result.backupsCreated());
                return result.outcome() == ProviderConfigMigrationService.Outcome.STALE ? 10 : 0;
            }
            var entries = service.inspect();
            runtime.terminal().stdout("PROVIDER_MIGRATION_RESULT=" + (dryRun || !prepare ? "DRY_RUN" : "DRY_RUN"));
            runtime.terminal().stdout("PROVIDERS_INSPECTED=" + entries.size());
            entries.forEach(entry -> runtime.terminal().stdout("PROVIDER_" + entry.provider().toUpperCase() + "=" + entry.outcome()));
            return 0;
        } catch (Exception failure) {
            runtime.terminal().stderr("PROVIDER_MIGRATION_RESULT=REQUIRES_HUMAN_REVIEW");
            return 10;
        }
    }
}
