...
public final class CollaborationContractBindCommand implements Callable<Integer> {
    ...
    @Option(names = "--revision", required = true) private long revision;
    /** Creates the command.
     * @param runtime CLI runtime
     */
    public CollaborationContractBindCommand(CliRuntime runtime) { this.runtime = runtime; }
    /** Executes binding. @return process exit code */
    @Override public Integer call() {
        try { new WorkspaceCollaborationService().bindContract(project.toAbsolutePath().normalize(), provider, connection, intent, contract, revision); runtime.terminal().stdout("CONTRACT_BOUND=" + contract + " REVISION=" + revision); return ExitCodes.OK; }
