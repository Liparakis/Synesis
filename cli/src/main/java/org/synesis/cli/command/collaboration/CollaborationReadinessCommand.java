...
public final class CollaborationReadinessCommand implements Callable<Integer> {
    ...
    @Option(names = "--control-head", required = true) private String controlHead;
    @Option(names = "--base", required = true) private String base;
    @Option(names = "--path", required = true) private List<String> paths;
    @Option(names = "--claim", required = true) private List<String> claims;
    @Option(names = "--tests-passed", defaultValue = "false") private boolean testsPassed;
    /** Creates the command.
     * @param runtime CLI runtime
     */
    public CollaborationReadinessCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            runtime.terminal().stdout("INTEGRATION_READY=" + result.accepted() + " FAILURES=" + result.failures() + " ACTIONS=" + result.actions());
            return result.accepted() ? ExitCodes.OK : ExitCodes.LOCAL_CONFIGURATION;
        } catch (Exception failure) { runtime.terminal().stderr("INTEGRATION_ERROR=" + failure.getMessage()); return ExitCodes.LOCAL_CONFIGURATION; }
    }
}
