...
@Command(name = "contract", description = "Publish, bind, and inspect shared contracts.", mixinStandardHelpOptions = true,
    subcommands = {CollaborationContractPublishCommand.class,
        CollaborationContractBindCommand.class,
...
public final class CollaborationContractCommand implements Runnable {
   ...
   */
  public CollaborationContractCommand(CliRuntime runtime) {
  ...
  public void run() {
    runtime.terminal()
        .stdout("Use collaboration contract publish, bind, or status.");
  }
}
