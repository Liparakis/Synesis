...
public final class PredictionListCommand implements Callable<Integer> {
     ...
     */
    public PredictionListCommand(CliRuntime runtime) {
        ...
        try {
            ...
            for (var event : CoordinationCliSupport.replay(endpoint, 0)) {
                if (event.type() == PredictionEventType.PREDICTION_CREATED) {
                    ids.put(event.predictionId(), true);
                }
            ...
            ids.keySet()
                    .forEach(id -> runtime.terminal()
                            .stdout("PREDICTION_ID=" + id + " STATE="
                                    + projection.state(id)
                                    .orElseThrow()));
            runtime.terminal()
                    .stdout("PREDICTIONS=" + ids.size());
            return ExitCodes.OK;
