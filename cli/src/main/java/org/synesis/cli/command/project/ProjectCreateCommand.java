...
public final class ProjectCreateCommand implements Callable<Integer> {
  ...
  @Option(names = "--peer", required = true, description = "Allowed peer node ID.")
  private String peer;
  @Option(names = "--project", description = "Explicit initialized project directory.")
   ...
   */
  public ProjectCreateCommand(CliRuntime runtime) {
  ...
  public Integer call() {
    try {
      ...
      var result = runtime.projectService()
          .createProject(location, peer);
      runtime.terminal()
          .stdout("NODE_ID=" + result.nodeId());
      runtime.terminal()
          .stdout("PROJECT_ID=" + result.projectId());
      runtime.terminal()
          .stdout("PEER_NODE_ID=" + result.peerNodeId());
      runtime.terminal()
          .stdout("PROJECT_CONFIGURED=true");
      return ExitCodes.OK;
