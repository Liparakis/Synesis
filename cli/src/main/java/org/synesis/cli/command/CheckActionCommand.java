...
public final class CheckActionCommand implements Callable<Integer> {
     ...
     */
    public CheckActionCommand(CliRuntime runtime) {
    ...
    public Integer call() {
        try {
            ...
            var result = runtime.guardrailService()
                    .check(resolved, location.root(), scope, "check-action", action);
            runtime.terminal()
                    .stdout("PROJECT_ID=" + location.projectId());
            if (result.outcome() == ActionGuardrail.Outcome.BLOCKED
                    || result.outcome() == ActionGuardrail.Outcome.WARNING) {
                ProjectConstraint c = result.matchedConstraint();
                runtime.terminal()
                        .stdout("ACTION_RESULT=" + (result.outcome() == ActionGuardrail.Outcome.BLOCKED ? "BLOCKED"
                                : "WARNING"));
                runtime.terminal()
                        .stdout("MATCHED_CONSTRAINT_ID=" + (c == null ? "" : c.recordId()));
                runtime.terminal()
                        .stdout("CONSTRAINT_EFFECT=" + (c == null ? "" : c.effect()));
                runtime.terminal()
                        .stdout("CONSTRAINT_TITLE=" + (c == null ? "" : c.title()));
                runtime.terminal()
                        .stdout("CONSTRAINT_RATIONALE=" + (c == null ? "" : c.rationale()));
                runtime.terminal()
                        .stdout("MATCHED_SCOPE=" + scope);
                runtime.terminal()
                        .stdout("REASON=" + result.message()
                                .replace('\n', ' '));
                if (result.outcome() == ActionGuardrail.Outcome.BLOCKED) {
                    runtime.terminal()
                            .stderr("HINT=Action blocked by project constraint. Re-plan required.");
                    return ExitCodes.LOCAL_CONFIGURATION;
                }
            } else {
                runtime.terminal()
                        .stdout("ACTION_RESULT=ALLOWED");
                runtime.terminal()
                        .stdout("MATCHED_CONSTRAINT_COUNT=0");
                runtime.terminal()
                        .stdout("REASON=No active project constraint collides with action.");
            }
            return ExitCodes.OK;
