...
public final class MigrateCommand implements Callable<Integer> {
  ...
  @Option(names = "--dry-run", description = "Inspect migration state without changing files.")
  @SuppressWarnings("unused")
  private boolean dryRun;
  @Option(names = "--prepare", description = "Create an immutable project migration plan.")
   ...
   */
  public MigrateCommand(CliRuntime runtime) {
  ...
  public Integer call() {
    try {
      ...
      if (prepare) {
        var plan = service.prepare(currentProject);
        runtime.terminal()
            .stdout("PROJECT_MIGRATION_RESULT=PLAN_PREPARED");
        runtime.terminal()
      ...
      if (showPlan != null) {
        ...
        runtime.terminal()
            .stdout("PROJECT_MIGRATION_RESULT=PLAN");
        runtime.terminal()
            .stdout("PLAN_ID=" + plan.planId());
        return 0;
      }
      if (execute != null) {
        ...
        runtime.terminal()
            .stdout("PROJECT_MIGRATION_RESULT=" + result.outcome());
        runtime.terminal()
            .stdout("REASON=" + result.reason());
        return switch (result.outcome()) {
          case FAILED, FAILED_RESTORED, REQUIRES_HUMAN_REVIEW, ROLLBACK_UNSAFE, STALE,
               UNSUPPORTED_SCHEMA -> 10;
          default -> 0;
        };
      }
      var entry = service.inspect(currentProject);
      runtime.terminal()
          .stdout("PROJECT_MIGRATION_RESULT=DRY_RUN");
      runtime.terminal()
          .stdout("PROJECT_SCHEMA=" + entry.sourceSchema());
      runtime.terminal()
          .stdout("PROJECT_MIGRATION_STATE=" + entry.outcome());
      return 0;
    } catch (Exception failure) {
      runtime.terminal()
          .stderr("PROJECT_MIGRATION_RESULT=REQUIRES_HUMAN_REVIEW");
      return 10;
    }
  }
}
