...
public final class SupervisorStatusCommand implements Callable<Integer> {
     ...
     */
    public SupervisorStatusCommand(CliRuntime runtime) {
    ...
    public Integer call() {
        try {
            var inspection = new IdentityBootstrap(profile.toAbsolutePath()
                    .normalize()
                    .resolve("link")).inspect();
            runtime.terminal()
                    .stdout("SUPERVISOR_PROFILE=" + profile.toAbsolutePath()
                            .normalize());
            runtime.terminal()
                    .stdout("NODE_ID=" + inspection.nodeId());
            runtime.terminal()
                    .stdout("IDENTITY=" + inspection.detail());
            runtime.terminal()
                    .stdout("CURSOR=" + CoordinationEventFollower.readCursor(profile.resolve("coordination.cursor")));
            return ExitCodes.OK;
        } catch (Exception failure) {
            runtime.terminal()
                    .stderr("SUPERVISOR_ERROR=" + failure.getMessage());
