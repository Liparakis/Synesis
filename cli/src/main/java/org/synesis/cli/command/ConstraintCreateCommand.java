...
public final class ConstraintCreateCommand implements Callable<Integer> {
    ...
    @Option(names = "--effect", defaultValue = "block")
     ...
     */
    public ConstraintCreateCommand(CliRuntime runtime) {
    ...
    public Integer call() {
        try {
            var location = runtime.projectService()
                    .require(project == null ? Path.of(".") : Path.of(project));
            if (profile != null) {
                location = new ProjectApplicationService.ProjectLocation(location.root(),
            ...
            var result = runtime.constraintService()
                    .create(location, title, rationale, scope, parsed);
            runtime.terminal()
                    .stdout("PROJECT_ID=" + result.projectId());
            runtime.terminal()
                    .stdout("RECORD_ID=" + result.recordId());
            runtime.terminal()
                    .stdout("DIGEST=" + result.digest());
            runtime.terminal()
                    .stdout("STATUS=" + result.status());
            runtime.terminal()
                    .stdout("CONSTRAINT_EFFECT=" + result.effect());
            runtime.terminal()
                    .stdout("CONSTRAINT_SCOPE=" + result.scope());
            runtime.terminal()
                    .stdout("SIGNATURE_VALID=true");
            return ExitCodes.OK;
        } catch (ConstraintApplicationService.ApplicationException failure) {
            runtime.terminal()
                    .stderr("ERROR=" + failure.code());
