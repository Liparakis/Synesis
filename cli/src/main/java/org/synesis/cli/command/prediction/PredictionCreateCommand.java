...
public final class PredictionCreateCommand implements Callable<Integer> {
  ...
  public Integer call() {
    try {
      ...
      PredictionContract contract = new PredictionContract(predictionId, location.projectId(),
          identity.nodeId(),
          supervisor, worker, taskId, capability, ownerNode, ownerSupervisor, scopes,
          projectSequence,
          baseCommit, baseScopeHashes, intentVersion, purpose, inputs, outputs, behavior, errors,
          sideEffects, invariants, compatibility, performance, concurrency, acceptanceTests,
          confidence,
          risk, expiresAt);
      var created = CoordinationCliSupport.submit(endpoint,
          CoordinationCommand.createAs(UUID.randomUUID(),
              location.projectId(), predictionId, PredictionEventType.PREDICTION_CREATED,
              identity.nodeId(),
              supervisor, worker, contract.encoded(), identity));
      var routed = CoordinationCliSupport.submit(endpoint,
          CoordinationCommand.createAs(UUID.randomUUID(),
              location.projectId(), predictionId, PredictionEventType.PREDICTION_ROUTED,
              identity.nodeId(),
