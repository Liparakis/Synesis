package org.synesis.cli.command.identity;
            ...
            try {
                Path selected = profile == null
                        ...
                        : Path.of(profile)
                          .toAbsolutePath()
                          .normalize();
                var identity = new IdentityBootstrap(selected.resolve("link")).loadOrCreate()
                        .identity();
                runtime.terminal()
                        .stdout("NODE_ID=" + identity.nodeId());
                return ExitCodes.OK;
            } catch (Exception failure) {
                runtime.terminal()
                        .stderr("ERROR=IDENTITY_FAILED");
