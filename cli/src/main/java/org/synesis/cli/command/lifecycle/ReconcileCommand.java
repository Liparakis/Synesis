...
/**
 * Runs the {@code synesis reconcile} lifecycle discovery, plan preparation, inspection, and
...
public final class ReconcileCommand implements Callable<Integer> {
   ...
   */
  public ReconcileCommand(CliRuntime runtime, ReconciliationService service,
  ...
  public Integer call() {
    if (!dryRun && !prepare && showPlan == null && executePlan == null) {
      runtime.terminal()
          .stderr(
              "Reconciliation execution is not available in this version without --dry-run, --prepare, --show-plan, or --execute.");
    ...
    try {
      ...
      if (verbose) {
        ...
        for (ReconciliationPlanEntry entry : plan.entries()) {
          runtime.terminal()
              .stdout(
                  "[" + entry.action() + "] target=" + entry.targetResourceId() + " | executable="
                      + entry.executable());
        }
        return ExitCodes.OK;
      }
  ...
  private int handleExecute(Path controlRoot, String planId) {
    try {
      ReconciliationService.ReconciliationExecutionSummary summary = service.executePlan(
          controlRoot, planId);
