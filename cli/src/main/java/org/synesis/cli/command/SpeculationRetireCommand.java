...
public final class SpeculationRetireCommand implements Callable<Integer> {
     ...
     */
    public SpeculationRetireCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            runtime.terminal().stdout("SPECULATION_RETIRED=true");
            return ExitCodes.OK;
    ...
    static void close(org.synesis.workspace.application.ProjectApplicationService.ProjectLocation location, UUID id)
            throws Exception {
        Path metadata = location.synesisDirectory().resolve("local").resolve("speculation").resolve(id.toString()).resolve("speculation.meta");
        String base = java.nio.file.Files.readAllLines(metadata).stream().filter(s -> s.startsWith("baseCommit=")).findFirst().orElseThrow().substring(11);
        new SpeculationWorkspace(location.root(), location.synesisDirectory().resolve("local"), id, base).close();
    }
}
