...
public final class CoordinationServeCommand implements Callable<Integer> {
    ...
    @Option(names = "--data", description = "Coordinator state directory.") private Path data;
    @Option(names = "--identity", description = "Coordinator identity directory.") private Path identity;
    @Option(names = "--host", defaultValue = "127.0.0.1") private String host;
    @Option(names = "--port", defaultValue = "48123") private int port;
     ...
     */
    public CoordinationServeCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            try (var server = new CoordinationHttpServer(service, new InetSocketAddress(host, port))) {
                server.start();
                runtime.terminal().stdout("COORDINATION_SERVE_READY endpoint=http://"
                        + server.address().getHostString() + ":" + server.address().getPort()
                        + "/ project=" + location.projectId() + " nodeId=" + node.nodeId());
                if (durationSeconds > 0) Thread.sleep(Duration.ofSeconds(durationSeconds).toMillis());
                else while (!Thread.currentThread().isInterrupted()) Thread.sleep(1000L);
            }
            return ExitCodes.OK;
        } catch (InterruptedException interrupted) {
            Thread.currentThread().interrupt(); return ExitCodes.OK;
