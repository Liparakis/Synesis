...
public final class SupervisorRunCommand implements Callable<Integer> {
     ...
     */
    public SupervisorRunCommand(CliRuntime runtime) {
    ...
    public Integer call() {
        try {
            ...
            runtime.terminal()
                    .stdout("SUPERVISOR_READY supervisor=" + supervisor + " worker=" + worker
                            + " nodeId=" + identity.nodeId() + " cursor=" + CoordinationEventFollower.readCursor(
                            cursorPath));
                    ...
                    event -> runtime.terminal()
                            .stdout(
                                    "EVENT sequence=" + event.sequence() + " type=" + event.type() + " prediction="
                                            + event.predictionId()));
            runtime.terminal()
                    .stdout("SUPERVISOR_STOPPED project=" + location.projectId()
