...
public final class EventsFollowCommand implements Callable<Integer> {
    ...
    @Option(names = "--cursor", required = true) private Path cursor;
     ...
     */
    public EventsFollowCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            CoordinationEventFollower.follow(endpoint, cursor, durationSeconds, event -> runtime.terminal().stdout(
                    "EVENT sequence=" + event.sequence() + " type=" + event.type() + " prediction=" + event.predictionId()));
            runtime.terminal().stdout("EVENTS_FOLLOW_COMPLETE project=" + location.projectId()
                    + " cursor=" + CoordinationEventFollower.readCursor(cursor));
            return ExitCodes.OK;
        } catch (Exception failure) {
            runtime.terminal().stderr("EVENTS_ERROR=" + failure.getMessage());
