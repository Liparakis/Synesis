...
public final class PredictionRespondCommand implements Callable<Integer> {
    ...
    @Option(names = "--action", required = true, description = "receive|exact|equivalent|revise|reject") private String action;
    @Option(names = "--reason", defaultValue = "") private String reason;
    private final CliRuntime runtime;
    /** Creates a prediction response command.
     * @param runtime composed CLI runtime
     */
    public PredictionRespondCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            runtime.terminal().stdout("PREDICTION_RESPONSE=" + type);
