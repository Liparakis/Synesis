...
import org.synesis.coordination.CoordinationTask;
import org.synesis.coordination.PredictionEventType;
import org.synesis.coordination.TaskClaim;
...
public final class TaskShowCommand implements Callable<Integer> {
     ...
     */
    public TaskShowCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            CoordinationTask task = null; TaskClaim claim = null;
            for (var event : CoordinationCliSupport.replay(endpoint, 0)) {
                if (!event.predictionId().equals(taskId)) continue;
                var command = CoordinationCommand.decode(event.payload());
                if (event.type() == PredictionEventType.TASK_CREATED) task = CoordinationTask.decode(command.payload());
                if (event.type() == PredictionEventType.TASK_CLAIMED) claim = TaskClaim.decode(command.payload());
                if (event.type() == PredictionEventType.TASK_RELEASED) claim = null;
            }
            if (task == null) throw new IllegalArgumentException("TASK_NOT_FOUND");
            runtime.terminal().stdout("TASK_ID=" + task.taskId());
            runtime.terminal().stdout("PROJECT_ID=" + location.projectId());
            runtime.terminal().stdout("TITLE=" + task.title());
            runtime.terminal().stdout("CAPABILITY=" + task.capability());
            runtime.terminal().stdout("OWNER_NODE=" + (claim == null ? "UNCLAIMED" : claim.ownerNodeId()));
            return ExitCodes.OK;
