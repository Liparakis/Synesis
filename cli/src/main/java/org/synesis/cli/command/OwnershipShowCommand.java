...
public final class OwnershipShowCommand implements Callable<Integer> {
     ...
     */
    public OwnershipShowCommand(CliRuntime runtime) { this.runtime = runtime; }
    /** Replays and prints one ownership claim.
     * @return stable process exit code
     */
    @Override public Integer call() {
        try {
            CoordinationCommand found = null; OwnershipClaim current = null;
            for (var event : CoordinationCliSupport.replay(endpoint, 0)) {
                if (event.type() != PredictionEventType.OWNERSHIP_CLAIMED
                        && event.type() != PredictionEventType.OWNERSHIP_RELEASED) continue;
                OwnershipClaim claim = OwnershipClaim.decode(CoordinationCommand.decode(event.payload()).payload());
                if (!claim.capability().equals(capability)) continue;
                found = CoordinationCommand.decode(event.payload()); current = event.type() == PredictionEventType.OWNERSHIP_CLAIMED
                        ? claim : null;
            }
            if (found == null || current == null) throw new IllegalArgumentException("OWNERSHIP_NOT_FOUND");
            runtime.terminal().stdout("CAPABILITY=" + current.capability());
            runtime.terminal().stdout("OWNER_NODE=" + current.ownerNodeId());
            runtime.terminal().stdout("OWNER_SUPERVISOR=" + current.ownerSupervisorId());
            runtime.terminal().stdout("TASK_ID=" + current.taskId());
            runtime.terminal().stdout("INTENT_VERSION=" + current.intentVersion());
            return ExitCodes.OK;
