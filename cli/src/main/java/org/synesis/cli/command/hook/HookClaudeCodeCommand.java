...
public final class HookClaudeCodeCommand implements Callable<Integer> {
     ...
     */
    public HookClaudeCodeCommand(CliRuntime runtime) {
    ...
    public Integer call() {
        try {
            ...
            var result = runtime.hookService()
                    .claudeCode(resolved, System.in);
            runtime.terminal()
                    .stdout(result.responseJson());
            if (result.humanReason() != null && !result.humanReason()
                    .isBlank()) {
                runtime.terminal()
                        .stderr("HINT=" + result.humanReason());
            }
            return ExitCodes.OK;
        } catch (ProjectApplicationService.ProjectApplicationException | RuntimeException failure) {
            runtime.terminal()
                    .stdout("{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"deny\",\"permissionDecisionReason\":\"Project is not initialized.\"}}");
