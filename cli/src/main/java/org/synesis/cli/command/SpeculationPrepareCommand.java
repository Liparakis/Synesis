...
public final class SpeculationPrepareCommand implements Callable<Integer> {
     ...
     */
    public SpeculationPrepareCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            SpeculationWorkspace workspace = new SpeculationWorkspace(location.root(),
                    location.synesisDirectory().resolve("local"), prediction, baseCommit);
            workspace.create();
            runtime.terminal().stdout("SPECULATION_PREPARED=true");
            runtime.terminal().stdout("PREDICTION_ID=" + prediction);
            runtime.terminal().stdout("WORKTREE=" + workspace.worktree());
            return ExitCodes.OK;
