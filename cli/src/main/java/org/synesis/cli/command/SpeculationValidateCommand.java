...
public final class SpeculationValidateCommand implements Callable<Integer> {
     ...
     */
    public SpeculationValidateCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            SpeculationWorkspace workspace = new SpeculationWorkspace(location.root(),
                    location.synesisDirectory().resolve("local"), prediction, readBaseCommit(location, prediction));
            SpeculationWorkspace.GateResult gate = workspace.gate();
            runtime.terminal().stdout(gate.status());
            if (!gate.accepted()) return ExitCodes.LOCAL_CONFIGURATION;
            ...
            runtime.terminal().stdout("VALIDATION_STARTED=true");
            return ExitCodes.OK;
    ...
    private static String readBaseCommit(org.synesis.workspace.application.ProjectApplicationService.ProjectLocation location,
        ...
        Path metadata = location.synesisDirectory().resolve("local").resolve("speculation").resolve(prediction.toString())
                .resolve("speculation.meta");
        for (String line : java.nio.file.Files.readAllLines(metadata)) if (line.startsWith("baseCommit=")) return line.substring(11);
