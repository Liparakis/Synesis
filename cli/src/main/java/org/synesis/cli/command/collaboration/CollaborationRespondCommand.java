...
public final class CollaborationRespondCommand implements Callable<Integer> {
    ...
    private CoordinationRequest.Status status;
    @Option(names = "--proposal", defaultValue = "")
     ...
     */
    public CollaborationRespondCommand(CliRuntime runtime) {
    ...
    public Integer call() {
        try {
            new WorkspaceCollaborationService().respond(project.toAbsolutePath()
                            .normalize(), provider, connection,
                    request, status, proposal);
            runtime.terminal()
                    .stdout("COORDINATION_RESPONDED=" + request);
            return ExitCodes.OK;
