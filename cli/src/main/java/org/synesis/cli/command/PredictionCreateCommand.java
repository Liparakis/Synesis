...
public final class PredictionCreateCommand implements Callable<Integer> {
    ...
    @Option(names = "--owner-node", required = true) private String ownerNode;
    @Option(names = "--owner-supervisor", required = true) private String ownerSupervisor;
    @Option(names = "--scope", required = true, split = ",") private List<String> scopes;
    @Option(names = "--base-commit", required = true) private String baseCommit;
    @Option(names = "--base-scope-hash", required = true, split = ",") private List<String> baseScopeHashes;
    @Option(names = "--project-sequence", defaultValue = "0") private long projectSequence;
    @Option(names = "--intent-version", defaultValue = "1") private long intentVersion;
    @Option(names = "--purpose", required = true) private String purpose;
    @Option(names = "--inputs", required = true) private String inputs;
    @Option(names = "--outputs", required = true) private String outputs;
    @Option(names = "--behavior", required = true) private String behavior;
    @Option(names = "--errors", required = true) private String errors;
    @Option(names = "--side-effects", required = true) private String sideEffects;
    @Option(names = "--invariants", required = true) private String invariants;
    @Option(names = "--compatibility", required = true) private String compatibility;
    @Option(names = "--performance", required = true) private String performance;
    @Option(names = "--concurrency", required = true) private String concurrency;
    @Option(names = "--acceptance-test", required = true, split = ",") private List<String> acceptanceTests;
    @Option(names = "--confidence", defaultValue = "80") private int confidence;
    @Option(names = "--risk", defaultValue = "20") private int risk;
    @Option(names = "--expires-at", description = "Epoch milliseconds; defaults to ten minutes from now.") private long expiresAt;
    private final CliRuntime runtime;
    /** Creates a prediction command.
     * @param runtime composed CLI runtime
     */
    public PredictionCreateCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            if (expiresAt == 0) expiresAt = System.currentTimeMillis() + 600_000L;
            ...
            runtime.terminal().stdout("PREDICTION_CREATED=true");
            runtime.terminal().stdout("PREDICTION_ID=" + predictionId);
            runtime.terminal().stdout("REQUEST_ID=" + created.eventId());
            runtime.terminal().stdout("CREATE_SEQUENCE=" + created.sequence());
            runtime.terminal().stdout("ROUTED_SEQUENCE=" + routed.sequence());
            return ExitCodes.OK;
