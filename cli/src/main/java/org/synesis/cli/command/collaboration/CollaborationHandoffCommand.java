...
public final class CollaborationHandoffCommand implements Callable<Integer> {
     ...
     */
    public CollaborationHandoffCommand(CliRuntime runtime) {
    ...
    public Integer call() {
        try {
            var request = new WorkspaceCollaborationService().handoff(project.toAbsolutePath()
                            .normalize(), provider,
                    connection, intent, target, proposal);
            runtime.terminal()
                    .stdout("HANDOFF_OFFERED=" + request.requestId());
            return ExitCodes.OK;
