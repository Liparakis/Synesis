...
public final class SpeculationRetireCommand implements Callable<Integer> {
  ...
  static void close(
      org.synesis.workspace.application.ProjectApplicationService.ProjectLocation location, UUID id)
      throws Exception {
    ...
    try {
      ...
      CoordinationCliSupport.submit(endpoint,
          CoordinationCommand.create(UUID.randomUUID(), location.projectId(),
              prediction, PredictionEventType.SPECULATION_RETIRED, identity.nodeId(), new byte[0],
              identity));
