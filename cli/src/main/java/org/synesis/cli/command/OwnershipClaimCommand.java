...
public final class OwnershipClaimCommand implements Callable<Integer> {
     ...
     */
    public OwnershipClaimCommand(CliRuntime runtime) { this.runtime = runtime; }
    ...
    @Override public Integer call() {
        try {
            ...
            var command = CoordinationCommand.createAs(UUID.randomUUID(), location.projectId(), taskId,
                    PredictionEventType.OWNERSHIP_CLAIMED, identity.nodeId(), supervisor, "owner", claim.encoded(), identity);
            var event = CoordinationCliSupport.submit(endpoint, command);
            runtime.terminal().stdout("OWNERSHIP_CLAIMED=true");
