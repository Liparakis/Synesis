...
/**
 * Installs project provider integrations for MCP tests without touching the developer's real
...
final class McpProviderTestSupport {
   ...
   */
  static void install(ProjectApplicationService.ProjectLocation location, String provider)
      throws Exception {
    ...
    try {
      ...
      if (result.exitCode() != 0 || !"BOUND".equals(result.values()
        ...
        throw new IllegalStateException(
            "Provider installation did not bind a session: " + result.values());
      }
    } finally {
