...
public record CoordinationTask(
    ...
    public byte[] encoded() {
        try {
            ...
            out.writeInt(MAGIC); out.writeInt(1); writeUuid(out, taskId); writeUuid(out, projectId);
            writeText(out, title); writeText(out, capability); writeText(out, creatorNodeId);
            writeText(out, creatorSupervisorId); writeText(out, creatorWorkerId); out.flush();
            if (bytes.size() > MAX_BYTES) throw new IllegalArgumentException("task exceeds bound");
    ...
    public static CoordinationTask decode(byte[] encoded) throws IOException {
        ...
        if (encoded.length > MAX_BYTES) throw new IOException("task exceeds bound");
        try {
            DataInputStream in = new DataInputStream(new ByteArrayInputStream(encoded));
            if (in.readInt() != MAGIC || in.readInt() != 1) throw new IOException("unsupported task format");
            ...
            if (in.available() != 0) throw new IOException("trailing task bytes");
            return task;
