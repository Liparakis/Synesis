  ...
  public CapabilityContract {
    ...
    if (inputs.isEmpty() || inputs.length() > MAX_FIELD_LENGTH) {
      throw new IllegalArgumentException(
          "inputs must be between 1 and " + MAX_FIELD_LENGTH + " characters");
    }
    if (output.isEmpty() || output.length() > MAX_FIELD_LENGTH) {
      throw new IllegalArgumentException(
          "output must be between 1 and " + MAX_FIELD_LENGTH + " characters");
    }
    ...
    if (requiredBehavior.size() > MAX_LIST_ITEMS) {
      throw new IllegalArgumentException(
          "requiredBehavior exceeds maximum item limit of " + MAX_LIST_ITEMS);
    }
    ...
    if (acceptanceTests.size() > MAX_LIST_ITEMS) {
      throw new IllegalArgumentException(
          "acceptanceTests exceeds maximum item limit of " + MAX_LIST_ITEMS);
    }
