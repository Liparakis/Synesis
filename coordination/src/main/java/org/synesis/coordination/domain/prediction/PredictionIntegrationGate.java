...
public final class PredictionIntegrationGate {
   ...
   */
  public static Result evaluate(boolean predictionResolved,
      SpeculationWorkspace.GateResult gitResult) {
    Objects.requireNonNull(gitResult, "git result");
    if (!predictionResolved) {
      return new Result(false, "INTEGRATION_GATE=REJECTED", "UNRESOLVED_PREDICTION");
    }
    if (!gitResult.accepted()) {
      return new Result(false, "INTEGRATION_GATE=REJECTED", gitResult.diagnostics());
    }
