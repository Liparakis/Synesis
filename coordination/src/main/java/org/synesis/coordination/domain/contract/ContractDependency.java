...
public record ContractDependency(UUID intentId, String participant, UUID contractId, long revision, State state) {
    ...
    public enum State {
        /** Consumer is bound to the revision. */ ACCEPTED,
        /** Consumer must revise its plan before proceeding. */ REPLAN_REQUIRED
    }
    ...
    public ContractDependency {
        Objects.requireNonNull(intentId, "intent ID"); Objects.requireNonNull(participant, "participant");
        Objects.requireNonNull(contractId, "contract ID"); Objects.requireNonNull(state, "state");
        if (revision < 1 || participant.isBlank()) throw new IllegalArgumentException("invalid dependency");
    }
}
