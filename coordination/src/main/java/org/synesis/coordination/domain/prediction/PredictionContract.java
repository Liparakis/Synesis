...
/**
 * Immutable, bounded description of the capability a requester predicts. Text is canonicalized as
 * strict UTF-8 and the encoded contract is safe to sign and persist.
 *
...
public record PredictionContract(
  ...
  public static PredictionContract decode(byte[] encoded) throws IOException {
    ...
    try {
      ...
      return new PredictionContract(predictionId, projectId, requesterNodeId, requesterSupervisorId,
          ...
          purpose, inputs, outputs, behavior, errorSemantics, sideEffects, invariants,
          compatibility,
          performance, concurrency, acceptanceTests, confidence, speculationRisk,
  ...
  public byte[] encoded() {
    try {
      ...
      String[] values = {requesterNodeId, requesterSupervisorId, requesterWorkerId,
          owningCapability,
          ownerNodeId, ownerSupervisorId, baseCommit, purpose, inputs, outputs, behavior,
          errorSemantics,
