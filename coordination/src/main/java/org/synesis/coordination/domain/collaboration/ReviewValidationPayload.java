...
public record ReviewValidationPayload(UUID grantId, UUID workGroupId, UUID targetIntentId,
    ...
    public ReviewValidationPayload {
        ...
        result = Objects.requireNonNull(result, "result").trim().toUpperCase(java.util.Locale.ROOT);
    ...
    public byte[] encode() {
        try {
            ...
            out.writeInt(MAGIC);
            uuid(out, grantId); uuid(out, workGroupId); uuid(out, targetIntentId);
            text(out, targetParticipant); out.writeLong(claimEpoch); uuid(out, taskId);
            text(out, snapshotId); text(out, result); text(out, reason); text(out, sourceParticipant);
            out.flush();
    ...
    public static ReviewValidationPayload decode(byte[] encoded) throws IOException {
        try {
            ...
            if (in.readInt() != MAGIC) throw new IOException("unsupported review validation format");
            ...
            if (in.available() != 0) throw new IOException("trailing review validation bytes");
            return payload;
    ...
    private static void text(DataOutputStream out, String value) throws IOException {
        ...
        if (bytes.length > MAX_TEXT) throw new IOException("text exceeds payload bound");
        out.writeInt(bytes.length); out.write(bytes);
    }
    private static String readText(DataInputStream in) throws IOException {
        int length = in.readInt();
        if (length < 0 || length > MAX_TEXT) throw new IOException("invalid review text length");
        byte[] bytes = in.readNBytes(length);
        if (bytes.length != length) throw new IOException("truncated review validation");
