...
public record TaskClaim(UUID taskId, String ownerNodeId, String ownerSupervisorId,
  ...
  public static TaskClaim decode(byte[] encoded) throws IOException {
    try {
      ...
      if (in.readInt() != MAGIC || in.readInt() != 1) {
        throw new IOException("unsupported claim format");
      }
      TaskClaim claim = new TaskClaim(new UUID(in.readLong(), in.readLong()),
          readText(in),
          readText(in),
          readText(in));
      if (in.available() != 0) {
        throw new IOException("trailing claim bytes");
      }
      return claim;
    } catch (RuntimeException | java.io.EOFException failure) {
      throw new IOException("malformed task claim", failure);
    }
  }
