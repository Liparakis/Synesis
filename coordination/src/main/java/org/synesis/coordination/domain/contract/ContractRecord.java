...
public record ContractRecord(UUID contractId, UUID projectId, long revision, String owner,
        String contentHash, String body, Status status, UUID supersedes, List<String> selectorRefs) {
    /** Contract lifecycle statuses. */
    public enum Status {
        /** Current usable revision. */ ACTIVE,
        /** Replaced by a newer revision. */ SUPERSEDED
    }
    ...
    public ContractRecord {
        Objects.requireNonNull(contractId, "contract ID"); Objects.requireNonNull(projectId, "project ID");
        Objects.requireNonNull(owner, "owner"); Objects.requireNonNull(contentHash, "content hash");
        Objects.requireNonNull(body, "body"); Objects.requireNonNull(status, "status");
        supersedes = supersedes;
        selectorRefs = List.copyOf(selectorRefs == null ? List.of() : selectorRefs);
        if (revision < 1 || owner.isBlank() || body.isBlank() || body.length() > 32768) throw new IllegalArgumentException("invalid contract");
        if (!contentHash.equals(hash(body))) throw new IllegalArgumentException("content hash mismatch");
    }
     ...
     */
    public static ContractRecord create(UUID contractId, UUID projectId, String owner, String body, List<String> selectors) {
