 ...
 * the control head from which the repair worktree was materialized, and both claim epochs.  The
 * decoder retains a bounded V1 reader solely so historical event logs remain replayable; new events
 * are always encoded as V2.</p>
 *
...
public record RepairLanePayload(UUID sourceIntentId, WorkIntent targetIntent,
  ...
  /**
   * Creates a metadata-free payload for callers that only replay historical transfer events.  New
   * repair operations should use the full constructor.
   *
  ...
  public static RepairLanePayload decode(byte[] encoded) throws IOException {
    try {
      ...
      if (magic != V1_MAGIC && magic != V2_MAGIC) {
        throw new IOException("unsupported repair payload");
      }
      UUID source = new UUID(in.readLong(), in.readLong());
      int length = in.readInt();
      if (length < 1 || length > 1024 * 1024) {
        throw new IOException("repair intent bound");
      }
      byte[] intent = in.readNBytes(length);
      if (intent.length != length) {
        throw new IOException("truncated repair payload");
      }
  ...
  private static void writeText(DataOutputStream out, String value) throws IOException {
    ...
    if (bytes.length > MAX_TEXT) {
      throw new IOException("repair text bound");
    }
  ...
  private static String readText(DataInputStream in) throws IOException {
    ...
    if (bytes.length != length) {
      throw new IOException("truncated repair text");
    }
  ...
  public byte[] encode() {
    try {
      ...
      out.writeInt(V2_MAGIC);
      out.writeLong(sourceIntentId.getMostSignificantBits());
      out.writeLong(sourceIntentId.getLeastSignificantBits());
      out.writeInt(intent.length);
      out.write(intent);
      writeText(out, snapshotId);
      writeText(out, expectedControlHead);
      out.writeLong(sourceClaimEpoch);
      out.writeLong(targetClaimEpoch);
      out.flush();
