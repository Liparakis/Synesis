...
public final class PredictionProjection {
  ...
  private static PredictionState transition(PredictionState current, PredictionEventType type) {
    if (current == null) {
      ...
      if (type == PredictionEventType.TASK_CREATED || type == PredictionEventType.TASK_CLAIMED
          || type == PredictionEventType.TASK_RELEASED
          || type == PredictionEventType.OWNERSHIP_CLAIMED
          ...
          || type == PredictionEventType.SESSION_FINALIZED
          || type == PredictionEventType.SESSION_ABANDONED
          ...
          || type == PredictionEventType.WORK_GROUP_CREATED
          || type == PredictionEventType.LANE_GRANT_ISSUED
          || type == PredictionEventType.LANE_GRANT_CONSUMED
          || type == PredictionEventType.LANE_REVOKED
          ...
          || type == PredictionEventType.PROVIDER_SESSION_TERMINALIZED
    ...
    return switch (type) {
      ...
      case ACCEPTED_EXACT ->
          require(current, PredictionState.ACCEPTED_EXACT, PredictionState.RECEIVED);
      case ACCEPTED_EQUIVALENT ->
          require(current, PredictionState.ACCEPTED_EQUIVALENT, PredictionState.RECEIVED);
      ...
      case IMPLEMENTATION_STARTED -> require(current, PredictionState.IMPLEMENTING,
          PredictionState.ACCEPTED_EXACT, PredictionState.ACCEPTED_EQUIVALENT,
          PredictionState.REVISED);
      case PATCH_READY ->
          require(current, PredictionState.PATCH_READY, PredictionState.IMPLEMENTING);
      case CAPABILITY_AVAILABLE ->
          require(current, PredictionState.AVAILABLE, PredictionState.PATCH_READY);
      case VALIDATION_STARTED ->
          require(current, PredictionState.VALIDATING, PredictionState.AVAILABLE);
      case SPECULATION_RETIRED ->
          require(current, PredictionState.RETIRED, PredictionState.VALIDATING);
      ...
      case TASK_CREATED, TASK_CLAIMED, TASK_RELEASED, OWNERSHIP_CLAIMED, OWNERSHIP_RELEASED,
           CAPABILITY_REQUEST_CREATED, CAPABILITY_REQUEST_CONTRACT_REVISED,
           ...
           REVIEW_VALIDATION_RECORDED, PROVIDER_SESSION_TERMINALIZED -> current;
    };
  }
  ...
  private static PredictionState requireNonTerminal(PredictionState current,
  ...
  private static boolean isNonPredictionEvent(PredictionEventType type) {
    return switch (type) {
      case TASK_CREATED, TASK_CLAIMED, TASK_RELEASED, OWNERSHIP_CLAIMED, OWNERSHIP_RELEASED,
           ...
           COMPLETION_UNWOUND, REVIEW_VALIDATION_RECORDED, PROVIDER_SESSION_TERMINALIZED -> true;
      default -> false;
    };
  }
