...
/**
 * Authenticated evidence for explicitly completing an intent without a repository snapshot.
 *
 * <p>The event sequence and the project identity are supplied by the event
 * store. The remaining fields bind the completion to the exact intent, participant, authority
 * lineage, workspace generation, and optimistic work-group/revision facts observed by the
 * caller.</p>
 *
...
public record NoChangeCompletion(
  ...
  public NoChangeCompletion {
    ...
    summary =
        summary == null || summary.isBlank() ? "Completed successfully without repository mutation"
            : summary.trim();
