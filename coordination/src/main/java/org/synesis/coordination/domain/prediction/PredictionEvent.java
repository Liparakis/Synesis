...
public final class PredictionEvent {
  ...
  public static PredictionEvent create(UUID projectId, UUID predictionId, long sequence,
    ...
    byte[] canonical = canonical(eventIdFor(sequence, projectId, predictionId), projectId,
        predictionId, sequence,
        type, actorNodeId, createdAtEpochMillis, payload, previousDigest);
    byte[] digest = sha256(canonical);
    return new PredictionEvent(eventIdFor(sequence, projectId, predictionId), projectId,
        predictionId, sequence,
        type, type.wireCode(), actorNodeId, createdAtEpochMillis, payload, previousDigest, digest,
        publicKey,
  ...
  public static PredictionEvent decode(byte[] encoded) throws IOException {
    ...
    try {
      ...
      byte[] previous = readBytes(in), digest = readBytes(in), key = readBytes(
          in), signature = readBytes(in);
      if (in.available() != 0) {
  ...
  private static PredictionEventType decodeWireType(int wireCode, byte[] payload)
      throws IOException {
    if (wireCode == 42 && (hasMagic(payload, 0x53494e31) || hasMagic(payload, 0x53494e32)
        ...
        || hasMagic(payload, 0x53494e35) || hasMagic(payload, 0x53494e36)
        || hasMagic(payload, 0x53494e37))) {
  ...
  public boolean verify() throws GeneralSecurityException {
    byte[] canonical = canonical(eventId, projectId, predictionId, sequence, encodedWireCode,
        actorNodeId,
        createdAtEpochMillis, payload, previousDigest);
    if (!Arrays.equals(digest, sha256(canonical))) {
      canonical = canonical(eventId, projectId, predictionId, sequence, type.wireCode(),
          actorNodeId,
          createdAtEpochMillis, payload, previousDigest);
      if (!Arrays.equals(digest, sha256(canonical))) {
