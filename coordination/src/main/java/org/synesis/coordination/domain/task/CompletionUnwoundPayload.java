...
public record CompletionUnwoundPayload(CompletionPreparedPayload prepared,
    ...
    public CompletionUnwoundPayload {
        ...
        if (!prepared.laneId().equals(replacementIntent.intentId())) {
    ...
    public static CompletionUnwoundPayload decode(byte[] encoded) throws IOException {
        ...
        try {
            ...
            if (in.readInt() != MAGIC) throw new IOException("unsupported unwind format");
            byte[] preparedBytes = readBytes(in);
            byte[] intentBytes = readBytes(in);
            if (in.available() != 0) throw new IOException("trailing unwind bytes");
    ...
    private static byte[] readBytes(DataInputStream in) throws IOException {
        ...
        if (length < 1 || length > 1_000_000) throw new IOException("payload bound");
