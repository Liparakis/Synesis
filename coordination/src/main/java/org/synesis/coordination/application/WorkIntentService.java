 ...
 * transitions acquire {@link ProjectAppendLock} around the complete read/validate/append sequence.
 * Claims therefore represent durable, authenticated ownership decisions rather than an in-memory
 * reservation; conflicts fail closed and do not grant authority to either contender.</p>
 */
...
public final class WorkIntentService {
   ...
   */
  public synchronized ClaimResult announce(WorkIntent intent)
  ...
  private void appendAutomaticConflictInbox(PredictionEventStore current, WorkIntent contender,
    ...
    for (ClaimConflict conflict : conflicts) {
      ...
      appendConflictRequest(current, contender.participant(), conflict.participant(),
          conflictingIntentId,
          proposal, contender.intentId() + "->" + conflictingIntentId);
      appendConflictRequest(current, conflict.participant(), contender.participant(),
          conflictingIntentId,
  ...
  private void appendConflictRequest(PredictionEventStore current, String requester, String target,
    ...
    CoordinationRequest request = new CoordinationRequest(requestId, current.projectId(), requester,
        target,
        conflictingIntentId, CoordinationRequest.Kind.CONTRACT, proposal,
        CoordinationRequest.Status.PENDING);
   ...
   * revision, different participant, different lineage, or non-integrated snapshot fails closed.
   * The event ordering is deliberately integration first, then intent release, so a rejected or
   * pending snapshot cannot terminalize its lane.</p>
   *
   * @param lock     already-held project append lock
   * @param snapshot exact snapshot whose integration succeeded
   * @return {@code true} when an active intent was released; {@code false} when the lane was
   * already released
   ...
   * without a snapshot. The caller supplies optimistic evidence, while this service verifies the
   * durable intent, group, epoch, and event revision under the project append lock. A repeated
   * identical completion is a read-only replay and never appends a second terminal event.</p>
   *
   ...
   * owner until the signed event is durably appended, and projection replay removes the source and
   * announces the target as one transition.  Repeating the same operation after a lost response is
   * idempotent.</p>
   *
  ...
  /**
   * Atomically transfers repair scope while the caller holds the project append lock used to
   * serialize Git integration and event publication.
   *
   * @param lock                already-held project append lock
   * @param sourceIntentId      published source lane
   * @param snapshotId          immutable conflicting snapshot identifier
  ...
  public CoordinationRequest request(String requester, UUID conflictingIntentId,
    ...
    try (ProjectAppendLock lock = ProjectAppendLock.acquire(store.rootDirectory())) {
      ...
      WorkIntent conflict = current.collaborationProjection()
          .intent(conflictingIntentId)
          .orElseThrow(() -> new IOException("INTENT_NOT_FOUND"));
      CoordinationRequest request = new CoordinationRequest(UUID.randomUUID(), current.projectId(),
          requester,
          conflict.participant(), conflictingIntentId, kind, proposal,
          CoordinationRequest.Status.PENDING);
      current.append(request.requestId(), PredictionEventType.COORDINATION_REQUESTED,
          signer.nodeId(),
          CollaborationCodec.encodeRequest(request), signer);
      return request;
    }
   ...
   * @throws GeneralSecurityException signing failure
   */
  public CoordinationRequest offerHandoff(String owner, UUID intentId, String target,
      String proposal)
      throws IOException, GeneralSecurityException {
    try (ProjectAppendLock lock = ProjectAppendLock.acquire(store.rootDirectory())) {
      if (!lock.isHeld()) {
   ...
   * @throws GeneralSecurityException signing failure
   */
  public void respond(String participant, UUID requestId, CoordinationRequest.Status status,
      String proposal)
      throws IOException, GeneralSecurityException {
    ...
    try (ProjectAppendLock lock = ProjectAppendLock.acquire(store.rootDirectory())) {
      ...
      if (status == CoordinationRequest.Status.ACCEPTED
          && request.kind() == CoordinationRequest.Kind.HANDOFF) {
   ...
   * @throws GeneralSecurityException signing failure
   */
  public void acknowledgeInbox(String participant, UUID itemId)
      throws IOException, GeneralSecurityException {
    try (ProjectAppendLock lock = ProjectAppendLock.acquire(store.rootDirectory())) {
      if (!lock.isHeld()) {
        throw new IOException("event append lock unavailable");
   ...
   * @throws GeneralSecurityException signing failure
   */
  public void resolveInbox(String participant, UUID itemId, CoordinationRequest.Status status,
      String proposal)
      throws IOException, GeneralSecurityException {
  ...
  public void heartbeat(String participant) throws IOException, GeneralSecurityException {
    try (ProjectAppendLock lock = ProjectAppendLock.acquire(store.rootDirectory())) {
      ...
      String durableParticipant = current.collaborationProjection()
          .participantKey(participant)
          .orElseThrow();
      current.append(UUID.nameUUIDFromBytes(
              durableParticipant.getBytes(java.nio.charset.StandardCharsets.UTF_8)),
  ...
  private static final class ParticipantState {
    ...
    private static void requireHeld(PredictionEventStore store, String participant)
        throws IOException {
