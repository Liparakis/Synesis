 ...
 * checked again while the project append lock is held. It prevents a stale lane, participant, claim
 * epoch, lineage, or selector set from being mutated by a delayed caller.</p>
 *
...
public record WorkIntentMutationPrecondition(
  ...
  public static WorkIntentMutationPrecondition capture(WorkIntent intent) {
    ...
    return new WorkIntentMutationPrecondition(intent.intentId(), intent.participant(),
        intent.version(),
