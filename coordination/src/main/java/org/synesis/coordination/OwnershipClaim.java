...
public record OwnershipClaim(UUID taskId, String capability, String ownerNodeId, String ownerSupervisorId,
    ...
    public OwnershipClaim {
        Objects.requireNonNull(taskId, "task ID"); requireText(capability, "capability");
        requireText(ownerNodeId, "owner node ID"); requireText(ownerSupervisorId, "owner supervisor ID");
        Objects.requireNonNull(protectedScopes, "protected scopes");
        if (protectedScopes.size() > MAX_LIST_ENTRIES) throw new IllegalArgumentException("protected scopes exceed bound");
        protectedScopes = List.copyOf(protectedScopes); protectedScopes.forEach(scope -> requireText(scope, "scope"));
        if (intentVersion < 0) throw new IllegalArgumentException("intent version must be non-negative");
    }
    ...
    public byte[] encoded() {
        try {
            ...
            out.writeLong(taskId.getLeastSignificantBits()); writeText(out, capability); writeText(out, ownerNodeId);
            writeText(out, ownerSupervisorId); out.writeInt(protectedScopes.size());
            for (String scope : protectedScopes) writeText(out, scope); out.writeLong(intentVersion); out.flush();
    ...
    public static OwnershipClaim decode(byte[] encoded) throws IOException {
        try {
            ...
            if (in.readInt() != MAGIC || in.readInt() != 1) throw new IOException("unsupported ownership format");
            UUID taskId = new UUID(in.readLong(), in.readLong()); String capability = readText(in);
            String node = readText(in); String supervisor = readText(in); int count = in.readInt();
            if (count < 0 || count > MAX_LIST_ENTRIES) throw new IOException("scope bound");
            java.util.ArrayList<String> scopes = new java.util.ArrayList<>(count);
            for (int i = 0; i < count; i++) scopes.add(readText(in));
            OwnershipClaim claim = new OwnershipClaim(taskId, capability, node, supervisor, scopes, in.readLong());
            if (in.available() != 0) throw new IOException("trailing ownership bytes"); return claim;
