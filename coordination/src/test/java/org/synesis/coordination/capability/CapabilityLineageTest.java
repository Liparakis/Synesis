...
final class CapabilityLineageTest {
  ...
  void intentCodecRetainsLineageAcrossReplay() throws Exception {
    ...
    WorkIntent intent = new WorkIntent(intentId, UUID.randomUUID(), "agt-owner", "codex",
        UUID.randomUUID(), "implement", "tests", "base",
  ...
  void capabilityRequestAndPublisherPayloadsRetainLineage() throws Exception {
    ...
    CapabilityContract contract = new CapabilityContract("Task", "TaskTracker", List.of("CRUD"),
        List.of("test"));
    CapabilityRequestPayload request = new CapabilityRequestPayload(handle,
        "task_tracker",
        "node-a",
        "sup-a",
        "worker-a",
    ...
    assertEquals(lineage,
        CapabilityRequestPayload.decode(request.encode())
            .authorityLineageId());

    ImplementationEventPayload publication = new ImplementationEventPayload(handle, lineage, 1,
        "base", "commit",
        List.of("src/task_tracker.py"), "published", "", "", List.of(), "");
    assertEquals(lineage,
        ImplementationEventPayload.decode(publication.encode())
            .authorityLineageId());
  }
  ...
  void snapshotProvenanceRetainsLineageForSuccessorResolution() throws Exception {
    UUID lineage = UUID.randomUUID();
    SnapshotProvenance provenance = new SnapshotProvenance(UUID.randomUUID(), UUID.randomUUID(),
        lineage,
        "agt-owner", "binding", 2, List.of("req_123456789012"), List.of(),
        List.of("PATH_EXACT:src/a.py"),
        "refs/synesis/snapshots/snap_1", "integrity", "artifacts");
    TaskSnapshotPayload payload = new TaskSnapshotPayload(UUID.randomUUID(), "snap_1", "node",
        "sup", "worker",
        "binding", "base", "commit", List.of("src/a.py"), List.of("req_123456789012"), "summary",
        provenance);

    assertEquals(lineage,
        TaskSnapshotPayload.decode(payload.encode())
            .provenance()
            .authorityLineageId());
  }
}
