...
$errors = 0
function Fail([string]$Message) { Write-Error $Message; $script:errors++ }
$path = Join-Path $RepositoryRoot 'docs/agent/DEFERRED.md'
if (-not (Test-Path $path -PathType Leaf)) { Fail 'Missing docs/agent/DEFERRED.md'; exit 1 }
...
if ($entries.Count -eq 0) { Fail 'No deferred entries found' }
$ids = @{}
foreach ($entry in $entries) {
    $id = $entry.Groups[1].Value
    if ($ids.ContainsKey($id)) { Fail "Duplicate deferred ID: $id" } else { $ids[$id] = $true }
    $body = $entry.Groups['body'].Value
    $statusMatch = [regex]::Match($body, '(?m)^\*\*Status:\*\*\s*(\S+)')
    if (-not $statusMatch.Success -or $allowed -notcontains $statusMatch.Groups[1].Value) { Fail "Invalid status: $id" }
    foreach ($field in $required) {
        if ($body -notmatch [regex]::Escape("**${field}:**")) { Fail "Missing field '$field': $id" }
    }
...
foreach ($file in $sourceFiles) {
    ...
    foreach ($todo in [regex]::Matches($fileText, 'TODO\(?(SL-D-\d{3})\)?')) {
        if (-not $ids.ContainsKey($todo.Groups[1].Value)) { Fail "TODO references missing deferred ID: $($todo.Groups[1].Value) in $($file.FullName)" }
    }
...
if ($text -notmatch '(?m)^## SL-D-001') { Fail 'NAT traversal entry missing' }
if ($text -notmatch '(?m)^## SL-D-026') { Fail 'reserved relay entry missing' }
