...
Branch pushes, tagged GitHub releases, and manual runs from the Actions **Run
workflow** button publish each row independently as a raw release asset named
by the runnable file (for example, `synesis-windows-x64.exe`). Branch pushes
use a numbered `build-<run-number>` prerelease. Manual runs may provide a
release tag such as `v0.1.0`; if left blank, the workflow creates a numbered
manual prerelease automatically. The file itself is the self-extracting installer with the CLI,
bundled Java runtime, native MCP launcher, and native installer; it is not
wrapped in a ZIP by the workflow. Pull-request runs build and smoke-test the
files but do not publish downloadable artifacts. No separate
