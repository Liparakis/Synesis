...
- Active task: STRUCT-1A ACTIVE under Reorganize Synesis package structure following CP-0210
...
- Exact next code action: Inventory `project-record`, `coordination`, and `link` foundational-package references, execute the `STRUCT-1A` package moves, run the required Gradle/bootstrap verification, commit `Reorganize Synesis foundational packages`, and create the `STRUCT-1A` checkpoint.
