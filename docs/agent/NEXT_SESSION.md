...
- Active task: SYN-015 ACTIVE under Reorganize Synesis package structure; `QUALITY-DEDUP` ACTIVE following CP-0217
- Repository branch: master
- Last checkpoint: CP-0217
- Last passing command: `:coordination:check :workspace:check :cli:check :mcp:check check`, `go test ./...`, and `go vet ./...`.
- Immediate next command: `powershell -ExecutionPolicy Bypass -File scripts/agent-resume.ps1`
- Exact next code action: Inventory and rank duplicated infrastructure/lifecycle logic, then extract only the first proven identical group after confirming the durable-state commit is clean.
