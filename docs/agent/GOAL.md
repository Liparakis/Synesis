...
- Current phase: staged package-structure refactor Phase 1 with `STRUCT-1C` complete and `STRUCT-1D` active; SYN-012 is complete at CP-0144; SYN-011 remains VERIFYING, SYN-010B remains VERIFYING, and SYN-010A remains VERIFYING
...
- Exact continuation: inventory CLI command FQNs and perform the first STRUCT-1D package move; do not begin deduplication before STRUCT-1D validation and checkpoint.
