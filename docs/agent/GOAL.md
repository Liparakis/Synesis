...
- Exact continuation: run `powershell -ExecutionPolicy Bypass -File
  scripts/agent-resume.ps1`, inspect the current MCP catalog and administrative
  state, and implement SYN-036 task 1 (catalog/rendering identities and
  administrative-state foundation). Keep `SYN-014E` paused and do not add
