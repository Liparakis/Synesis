...
final class CandidateRacerTest {
  ...
  private static CandidatePair pair(String address, int priority) throws Exception {
    Candidate local = new Candidate(CandidateType.MANUAL, InetAddress.getByName(address), 4433,
        priority);
    Candidate remote = new Candidate(CandidateType.MANUAL, InetAddress.getByName("198.51.100.2"),
        4433, priority);
  ...
  private static PeerSession usableSession() {
    ...
    session.attachControl(new PeerSession.ControlBinding() {
      ...
      @Override
      public java.util.concurrent.CompletionStage<Void> closeGracefully(SessionCloseReason reason) {
        terminal.complete(null);
        return terminal;
      }

      @Override
      public java.util.concurrent.CompletionStage<Void> terminalCompletion() {
        return terminal;
      }
    ...
    try (CandidateRacer racer = new CandidateRacer(new ConnectionPolicy(
      ...
      CandidateRacer.Operation operation = racer.race(List.of(first, second), "remote", pair -> {
        if (pair.equals(first)) {
          return new StubAttempt(pending, cancelled);
        }
        return new StubAttempt(CompletableFuture.completedFuture(winner), cancelled);
      });
      DirectConnectionResult result = operation.completion()
          .toCompletableFuture()
  ...
  private record StubAttempt(CompletableFuture<PeerSession> result, AtomicInteger cancelled)
    ...
    @Override
    public CompletableFuture<PeerSession> connect(CandidateCancellation cancellation) {
      return result;
    }
