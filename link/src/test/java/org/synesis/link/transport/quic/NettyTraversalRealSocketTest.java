...
final class NettyTraversalRealSocketTest {
  ...
  void exchangesOfferAndAnswerOnTheSameBoundUdpParents() throws Exception {
    ...
    try {
      ...
      CandidateDescriptor initiatorDescriptor = descriptor(initiator, issued, expires,
          initiatorPort);
      CandidateDescriptor responderDescriptor = descriptor(responder, issued, expires,
          responderPort);
      ...
      responderExchange = new NettyTraversalExchange(responderChannel, Duration.ofSeconds(2),
          received -> {
            try {
              if (!received.verifyAt(Instant.now(), TraversalOffer.DEFAULT_CLOCK_SKEW,
                  responder.nodeId())) {
                return null;
              }
      ...
      TraversalAnswer answer = initiatorExchange.sendOffer(offer,
