...
final class ApplicationStreamCodecTest {
  ...
  void roundTripsOpaquePayload() {
    ...
    assertArrayEquals(payload,
        ApplicationStreamCodec.decode(ApplicationStreamCodec.encode(payload)));
  }

  @Test
  void rejectsOversizedAndMalformedFrames() {
    assertThrows(IllegalArgumentException.class,
        () -> ApplicationStreamCodec.encode(
            new byte[ApplicationStreamCodec.MAX_PAYLOAD_BYTES + 1]));
    ...
    assertThrows(IllegalArgumentException.class,
        () -> ApplicationStreamCodec.decode(Arrays.copyOf(malformed,
            ApplicationStreamCodec.MAX_FRAME_BYTES + 1)));
  }
}
