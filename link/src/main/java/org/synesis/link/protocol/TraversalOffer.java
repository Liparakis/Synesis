 ...
 * assertion. The durable initiator identity, expected responder, invitation
 * binding, session, expiry, and fresh attempt nonce are all covered by the
 * Ed25519 signature.
 *
 * @since 1.0
 */
public final class TraversalOffer {
    ...
    private TraversalOffer(ProtocolVersion protocolVersion, UUID sessionId, String initiatorNodeId,
            ...
            Instant issuedAt, Instant expiresAt, byte[] attemptNonce, byte[] descriptor, byte[] signature) {
        ...
        this.initiatorPublicKey = boundedBytes(initiatorPublicKey, MAX_KEY_BYTES, "initiator public key");
     ...
     * @param expectedResponderNodeId optional expected durable responder ID;
     *                               {@code null} permits any authenticated responder
