 ...
 * and public key. A verifier must compare all expected transcript fields; a valid signature alone
 * is not proof that the proof belongs to the current connection. Instances are immutable and
 * thread-safe. Challenge and signature bytes are not safe to expose in default logs.
 *
 * @since 1.0
 */
public final class HandshakeProof {
   ...
   */
  public static HandshakeProof create(NodeIdentity identity, ProtocolVersion version,
      UUID sessionId,
  ...
  public static HandshakeProof decode(byte[] encoded) throws IOException {
    ...
    try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(encoded))) {
      ...
      String nodeId = new String(readBytes(input, MAX_NODE_ID_BYTES),
   ...
   * @throws NullPointerException     if an argument is {@code null}
   */
  public boolean verify(ProtocolVersion expectedVersion, UUID expectedSessionId,
