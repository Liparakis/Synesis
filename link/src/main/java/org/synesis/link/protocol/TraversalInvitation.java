...
/**
 * Human-mediated first traversal link containing the existing invitation and
 * a host-signed traversal offer.
 *
 * <p>The wrapper adds no trust of its own. The embedded {@link SessionInvitation}
 * and {@link TraversalOffer} remain independently canonical and signed, while
 * the offer digest binds them to the exact invitation bytes.
 *
 * @since 1.0
 */
public final class TraversalInvitation {
    ...
    private TraversalInvitation(SessionInvitation invitation, TraversalOffer offer) {
        ...
        try {
            ...
            throw new IllegalArgumentException("SLO1 contains an invalid invitation descriptor", exception);
        }
    }
    ...
    public String shareLink() {
        return "synesis://join/SLO1-" + Base64.getUrlEncoder().withoutPadding().encodeToString(encoded());
    }
