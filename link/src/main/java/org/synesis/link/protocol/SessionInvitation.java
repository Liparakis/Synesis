 ...
 * bound to the host descriptor, session ID, protocol version, and validity interval by the host
 * Ed25519 signature. Possession authorizes one admission attempt only; mutual Synesis identity
...
public final class SessionInvitation {
  ...
  public static SessionInvitation create(NodeIdentity host, UUID sessionId, ProtocolVersion version,
    ...
    byte[] cap = Objects.requireNonNull(capability, "capability")
        .clone();
    SessionInvitation unsigned = new SessionInvitation(version, sessionId, canonicalIssuedAt,
        canonicalExpiresAt, cap,
        descriptor.encoded(), new byte[]{1});
    return new SessionInvitation(version, sessionId, canonicalIssuedAt, canonicalExpiresAt, cap,
        descriptor.encoded(),
