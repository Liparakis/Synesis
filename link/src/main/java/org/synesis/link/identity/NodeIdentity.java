 ...
 * key objects supplied at construction; callers must not mutate provider objects outside the JDK
 * key contract. Private key material is never included in {@link #toString()} or returned as an
 * encoded value.
 *
 * @since 1.0
 */
public final class NodeIdentity {
  ...
  private NodeIdentity(KeyPair keyPair) {
    ...
    if (isUnsupportedAlgorithm(publicKey.getAlgorithm()) || isUnsupportedAlgorithm(
    ...
    if (publicKeyEncoded.length == 0 || publicKeyEncoded.length > MAX_PUBLIC_KEY_BYTES
        ...
        || privateKey.getEncoded().length > MAX_PRIVATE_KEY_BYTES) {
