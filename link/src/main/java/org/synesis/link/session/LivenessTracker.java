 ...
 * the current session and heartbeat sequence. Local writes never call it. Delayed callbacks derive
 * state from elapsed monotonic time, so scheduler pauses cannot leave an expired session in
 * SUSPECT.</p>
 */
public final class LivenessTracker {
  ...
  public void validPeerActivity() {
    ...
    synchronized (lock) {
      ...
      lastActivity = clock.nanoTime();
      if (state == LivenessState.SUSPECT) {
        transition = transitionLocked(LivenessState.LIVE, 0);
      }
    }
    if (transition != null) {
  ...
  public void stop(LivenessState terminal) {
    ...
    synchronized (lock) {
      ...
      if (!isTerminal(terminal)) {
        throw new IllegalArgumentException("stop requires terminal state");
      }
      running = false;
      if (scheduled != null) {
        scheduled.cancel();
  ...
  public LivenessMetrics metrics() {
    synchronized (lock) {
      ...
      return new LivenessMetrics(heartbeatSent, heartbeatReceived, heartbeatAcknowledged,
          duplicateOrStale, sendFailures, Duration.ofNanos(age),
          Duration.ofNanos(Math.max(0, rttNanos)),
  ...
  private void tick() {
    ...
    synchronized (lock) {
      ...
      if (elapsed >= configuration.expiryTimeout()
          .toNanos()) {
        if (state == LivenessState.LIVE) {
          first = transitionLocked(LivenessState.SUSPECT, elapsed);
        }
        if (state == LivenessState.SUSPECT) {
          second = transitionLocked(LivenessState.EXPIRED, elapsed);
        }
        running = false;
        expire = true;
      } else {
        if (elapsed >= configuration.suspicionTimeout()
            .toNanos() && state == LivenessState.LIVE) {
          first = transitionLocked(LivenessState.SUSPECT, elapsed);
        }
        if (running) {
          scheduleLocked();
  ...
  private LivenessTransition transitionLocked(LivenessState next, long elapsed) {
    ...
    state = next;
    if (previous == LivenessState.LIVE && next == LivenessState.SUSPECT) {
      liveToSuspect++;
    }
    if (previous == LivenessState.SUSPECT && next == LivenessState.LIVE) {
      suspectToLive++;
    }
    if (previous == LivenessState.SUSPECT && next == LivenessState.EXPIRED) {
      suspectToExpired++;
    }
    if (isTerminal(next)) {
      terminalTransitions++;
