 ...
 * node proof, so a proof cannot be moved to another session, role, ALPN, epoch, nonce, or identity
...
public final class HandshakeTranscript {
  ...
  private HandshakeTranscript(ProtocolVersion version, String alpn, UUID sessionId,
    ...
    this.initiatorPublicKey = copyBounded(initiatorPublicKey, MAX_KEY_BYTES,
        "initiator public key");
    this.responderNodeId = requireText(responderNodeId, MAX_NODE_ID_BYTES, "responder node ID");
    this.responderPublicKey = copyBounded(responderPublicKey, MAX_KEY_BYTES,
  ...
  public static HandshakeTranscript create(ProtocolVersion version, String alpn, UUID sessionId,
      ...
      String initiatorNodeId, byte[] initiatorPublicKey, String responderNodeId,
      byte[] responderPublicKey) {
    return create(version, alpn, sessionId, initiatorEpoch, responderEpoch, initiatorNonce,
        responderNonce,
        new byte[0], initiatorNodeId, initiatorPublicKey, responderNodeId, responderPublicKey);
  }

  /**
   * Creates a transcript with an optional invitation capability bound into the signed proof
   * challenge.
   *
   * @param version              protocol version
   * @param alpn                 negotiated application protocol
  ...
  public static HandshakeTranscript decode(byte[] value) throws IOException {
    ...
    try (DataInputStream input = new DataInputStream(new java.io.ByteArrayInputStream(value))) {
      ...
      HandshakeTranscript transcript = create(version, alpn, sessionId, initiatorEpoch,
          responderEpoch,
          initiatorNonce, responderNonce, invitationCapability, initiatorNodeId, initiatorPublicKey,
          responderNodeId, responderPublicKey);
