 ...
 * publish a session. QUIC channels, TLS engines, native handles, and private key material are
 * deliberately absent from this API. Mutable liveness and stream lifecycle APIs are added only by
 * their corresponding later slices.
 *
 * <p>The session is the authenticated identity boundary: the remote node ID,
 * public key, session ID, and epochs belong to this established session and must not be inferred
 * from a route, channel, or later connection. Control attachment is single-assignment; the volatile
 * bindings publish lifecycle state without exposing transport implementation objects.</p>
 *
 * @since 1.0
 */
public final class PeerSession {
   ...
   * Synesis cooperation API. It is non-blocking, bounded by the transport binding, and rejects use
   * before authentication/readiness or after close.
   *
   ...
   * @throws IllegalStateException if the session is not usable or has no application-stream
   *                               binding
   */
  ...
  /**
   * Returns the current liveness state; an unattached authenticated snapshot remains
   * {@link LivenessState#CONNECTING}.
   *
  ...
  /**
   * Registers a liveness listener through the authenticated control stream. Listener callbacks are
   * bounded and isolated from protocol state.
   *
  ...
  /**
   * Transport-neutral lifecycle bridge owned by the authenticated control stream. Implementations
   * must serialize writes on their transport event loop.
   */
