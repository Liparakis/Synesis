 ...
 * trust-on-first-use. The same transcript fingerprint cannot be accepted twice by one
 * {@link ReplayGuard}.
 *
 * @since 1.0
 */
public final class SessionAuthenticator {
  ...
  public static PeerSession establish(NodeIdentity localIdentity, String expectedRemoteNodeId,
    ...
    if (!localIdentity.nodeId()
        ...
        || !java.util.Arrays.equals(localIdentity.publicKeyEncoded(),
        transcript.publicKeyEncoded(localRole))) {
    ...
    if (expectedRemoteNodeId != null && !remoteProof.nodeId()
        ...
        || !java.util.Arrays.equals(remoteProof.publicKeyEncoded(),
        transcript.publicKeyEncoded(remoteRole))) {
    ...
    return new PeerSession(localIdentity.nodeId(), remoteNodeId,
        ...
        transcript.epoch(localRole), transcript.epoch(remoteRole), transcript.version(),
        establishedAt);
  }

  private static void verifyProof(HandshakeProof proof, HandshakeTranscript transcript,
      HandshakeRole role)
