 ...
 * not implement credentials, TURN allocation, relay behavior, or arbitrary
 * STUN extensions. The surrounding provider supplies transport deadlines and
 * cancellation.
 */
    ...
    public static InetSocketAddress xorMappedAddress(byte[] response, byte[] transactionId)
        ...
        while (input.position() < end) {
            ...
            if (type == XOR_MAPPED_ADDRESS || (type == MAPPED_ADDRESS && mapped == null)) {
                InetSocketAddress candidate = decodeAddress(value, type == XOR_MAPPED_ADDRESS, transactionId);
