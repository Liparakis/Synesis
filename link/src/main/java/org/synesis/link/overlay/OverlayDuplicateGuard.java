...
public final class OverlayDuplicateGuard {
  ...
  public synchronized boolean firstSeen(OverlayForwardingFrame frame, Instant now) {
    ...
    prune(now);
    Key key = new Key(frame.projectId(), frame.messageId(),
        OverlayCodecSupport.sha256(frame.innerRecord()));
  ...
  private record Key(UUID projectId, UUID messageId, byte[] digest) {
    ...
    public boolean equals(Object other) {
      return other instanceof Key key && projectId.equals(key.projectId) && messageId.equals(
          key.messageId)
