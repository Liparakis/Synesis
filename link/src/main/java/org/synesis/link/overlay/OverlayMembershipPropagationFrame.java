...
/**
 * Bounded hop-local wrapper for forwarding one signed {@code SLM1} membership snapshot through
 * authenticated direct peers.
 *
 * <p>The snapshot remains byte-for-byte unchanged. Only the propagation budget
 * is consumed at each hop. The wrapper is limited to the existing Link application-frame bound so
 * it cannot create a larger transport message.
 */
public final class OverlayMembershipPropagationFrame {
  ...
  private OverlayMembershipPropagationFrame(OverlayMembershipSnapshot membership,
      int remainingHops) {
  ...
  public static OverlayMembershipPropagationFrame create(OverlayMembershipSnapshot membership,
    ...
    if (remainingHops <= 0) {
      throw new IllegalArgumentException(
          "new membership propagation requires a positive hop budget");
    }
  ...
  public static OverlayMembershipPropagationFrame decode(byte[] bytes) throws IOException {
    ...
    try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      String authorityNodeId = OverlayCodecSupport.nodeId(
