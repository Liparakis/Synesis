...
/**
 * Propagates signed topology advertisements over currently authenticated direct project peers with
 * bounded fan-out and hop count.
 *
 * <p>This component does not create sockets, elect an authority, or relay
 * application payloads. {@link OverlayTopologyView} remains the authenticated local topology read
 * model.
 */
public final class OverlayTopologyPropagation {
  ...
  public OverlayTopologyPropagation(String localNodeId, OverlayMembershipSnapshot membership,
    ...
    this(localNodeId, new OverlayMembershipView(membership), topology,
        new OverlayPeerRegistry<>(peers));
  }
   ...
   * @throws GeneralSecurityException if the advertisement is not current or does not originate
   *                                  locally
   */
   ...
   * @param now                   current time
   * @return completion of all attempted peer sends
   * @throws GeneralSecurityException if sender or topology authentication is invalid
   */
  public CompletionStage<Void> receive(String immediateSenderNodeId,
      OverlayTopologyPropagationFrame frame,
      Instant now) throws GeneralSecurityException {
    Objects.requireNonNull(immediateSenderNodeId, "immediate sender node ID");
    Objects.requireNonNull(frame, "frame");
    Objects.requireNonNull(now, "now");
    OverlayMembershipSnapshot currentMembership = membership.current();
    if (!currentMembership.allows(immediateSenderNodeId) || !peers.contains(
        immediateSenderNodeId)) {
      throw new GeneralSecurityException("topology sender is not a direct project peer");
    }
    if (topology.accept(frame.advertisement(), currentMembership, now)
        != OverlayTopologyView.Acceptance.ACCEPTED
  ...
  private CompletionStage<Void> sendToPeers(OverlayTopologyPropagationFrame frame,
      String excludedPeer) {
