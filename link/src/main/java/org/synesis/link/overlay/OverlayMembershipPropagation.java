...
/**
 * Propagates signed membership snapshots over currently authenticated direct
 * project peers with bounded fan-out and hop count.
 *
 * <p>Only the declared membership authority may originate a new snapshot.
 * Receivers verify the signed snapshot through {@link OverlayMembershipView}
 * before forwarding it, so propagation does not create a second authority.
 */
public final class OverlayMembershipPropagation {
     ...
     * @throws GeneralSecurityException if the local node is not the authority
     *                                  or the snapshot is not accepted
     */
     ...
     * @param now current time
     * @return completion of all attempted peer sends
     * @throws GeneralSecurityException if sender or membership authentication
     *                                  is invalid
     */
    public CompletionStage<Void> receive(String immediateSenderNodeId, OverlayMembershipPropagationFrame frame,
    ...
    private CompletionStage<Void> sendToPeers(OverlayMembershipPropagationFrame frame, String excludedPeer) {
