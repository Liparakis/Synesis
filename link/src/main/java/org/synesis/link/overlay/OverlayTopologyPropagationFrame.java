...
/**
 * Bounded hop-local wrapper for forwarding one signed {@code SLT1} topology
 * advertisement through authenticated direct peers.
 *
...
public final class OverlayTopologyPropagationFrame {
    ...
    private OverlayTopologyPropagationFrame(OverlayTopologyAdvertisement advertisement, int remainingHops) {
    ...
    public static OverlayTopologyPropagationFrame decode(byte[] bytes) throws IOException {
        ...
        try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
            ...
            String originNodeId = OverlayCodecSupport.nodeId(input.readNBytes(OverlayCodecSupport.NODE_ID_BYTES));
            long sequence = input.readLong();
            int remainingHops = input.readUnsignedByte();
            int length = input.readUnsignedShort();
            if (length <= 0 || length > OverlayTopologyAdvertisement.MAX_BYTES || length > input.available()) {
            ...
            OverlayTopologyAdvertisement advertisement = OverlayTopologyAdvertisement.decode(advertisementBytes);
