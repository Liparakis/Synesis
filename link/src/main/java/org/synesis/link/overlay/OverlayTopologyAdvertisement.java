...
public final class OverlayTopologyAdvertisement {
    ...
    private OverlayTopologyAdvertisement(UUID projectId, String originNodeId, long membershipRevision, long sequence,
     ...
     */
    public static OverlayTopologyAdvertisement create(UUID projectId, long membershipRevision, long sequence,
            Instant expiresAt, List<String> adjacentNodeIds, NodeIdentity origin) throws GeneralSecurityException {
        Objects.requireNonNull(origin, "origin identity");
        List<String> normalized = normalizeNeighbors(origin.nodeId(), adjacentNodeIds);
        byte[] unsigned = encodeUnsigned(projectId, origin.nodeId(), membershipRevision, sequence, expiresAt,
                normalized);
        return new OverlayTopologyAdvertisement(projectId, origin.nodeId(), membershipRevision, sequence, expiresAt,
    ...
    public static OverlayTopologyAdvertisement decode(byte[] bytes) throws IOException {
        ...
        try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
            ...
            for (int index = 0; index < count; index++) {
                neighbors.add(OverlayCodecSupport.nodeId(input.readNBytes(OverlayCodecSupport.NODE_ID_BYTES)));
            }
            ...
            byte[] unsigned = encodeUnsigned(project, origin, membershipRevision, sequence, expires, normalized);
     ...
     */
    public void verifyAgainst(OverlayMembershipSnapshot membership, Instant now) throws GeneralSecurityException {
        Objects.requireNonNull(membership, "membership");
        Objects.requireNonNull(now, "now");
        if (!membership.projectId().equals(projectId) || membership.revision() != membershipRevision
                || !membership.isUsableAt(now) || !now.isBefore(expiresAt) || !membership.allows(originNodeId)) {
