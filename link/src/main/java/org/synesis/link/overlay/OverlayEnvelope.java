 ...
 * authenticated data for the AEAD operation and remain unchanged across
 * physical hops.
 */
public final class OverlayEnvelope {
    ...
    private OverlayEnvelope(UUID projectId, UUID messageId, String originNodeId, String destinationNodeId,
    ...
    static OverlayEnvelope sealed(UUID projectId, UUID messageId, String originNodeId, String destinationNodeId,
            UUID sessionId, long sequence, byte[] ciphertext) {
        return new OverlayEnvelope(projectId, messageId, originNodeId, destinationNodeId, sessionId, sequence,
                ciphertext);
    }
    ...
    public static OverlayEnvelope decode(byte[] bytes) throws IOException {
        ...
        try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
            ...
            OverlayEnvelope result = new OverlayEnvelope(project, message, origin, destination, session, sequence,
                    ciphertext);
    ...
    static byte[] associatedData(UUID projectId, UUID messageId, String originNodeId, String destinationNodeId,
    ...
    private byte[] encodeComplete() {
        try {
            ...
            try (DataOutputStream output = new DataOutputStream(bytes)) {
                output.write(associatedData(projectId, messageId, originNodeId, destinationNodeId, sessionId,
    ...
    public byte[] associatedData() {
        return associatedData(projectId, messageId, originNodeId, destinationNodeId, sessionId, sequence,
                ciphertext.length);
    }
