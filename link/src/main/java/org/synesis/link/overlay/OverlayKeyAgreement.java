 ...
 * Only the origin and destination retain the ephemeral private keys and the
 * derived session key.
 */
public final class OverlayKeyAgreement {
    ...
    private static final byte[] ORIGIN_TO_DESTINATION = "origin-to-destination".getBytes(StandardCharsets.US_ASCII);
    private static final byte[] DESTINATION_TO_ORIGIN = "destination-to-origin".getBytes(StandardCharsets.US_ASCII);
    ...
    public static Initiator start(UUID projectId, String destinationNodeId, UUID sessionId,
        ...
        InitRecord unsignedRecord = InitRecord.unsigned(projectId, origin.nodeId(), destinationNodeId, sessionId,
    ...
    public static Responder respond(InitRecord init, NodeIdentity destination,
        ...
        ResponseRecord response = ResponseRecord.signed(unsigned, destination.sign(unsigned.unsignedEncoded()));
        E2eSession session = deriveSession(init, response, ephemeral.getPrivate(), init.ephemeralPublicKey(), false);
    ...
    public static E2eSession finish(Initiator initiator, ResponseRecord response,
        ...
        return deriveSession(init, response, initiator.ephemeralPrivateKey, response.ephemeralPublicKey(), true);
    }

    private static void validateMembership(UUID projectId, long revision, String origin, String destination,
            OverlayMembershipSnapshot membership) throws GeneralSecurityException {
    ...
    private static E2eSession deriveSession(InitRecord init, ResponseRecord response, PrivateKey privateKey,
        ...
        return new E2eSession(init.projectId(), init.sessionId(), sendOrigin, sendDestination, receiveOrigin,
    ...
    public static final class InitRecord {
        ...
        private InitRecord(UUID projectId, String originNodeId, String destinationNodeId, UUID sessionId,
            ...
            this.ephemeralPublicKey = fixed(ephemeralPublicKey, OverlayCodecSupport.X25519_PUBLIC_KEY_BYTES,
        ...
        private static InitRecord unsigned(UUID projectId, String originNodeId, String destinationNodeId,
                UUID sessionId, long membershipRevision, byte[] nonce, byte[] ephemeralPublicKey) {
            return new InitRecord(projectId, originNodeId, destinationNodeId, sessionId, membershipRevision, nonce,
        ...
        public static InitRecord decode(byte[] bytes) throws IOException {
            ...
            try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
                ...
                String origin = OverlayCodecSupport.nodeId(input.readNBytes(OverlayCodecSupport.NODE_ID_BYTES));
                String destination = OverlayCodecSupport.nodeId(input.readNBytes(OverlayCodecSupport.NODE_ID_BYTES));
                ...
                InitRecord result = new InitRecord(project, origin, destination, session, revision, nonce, ephemeral,
                        signature);
         ...
         */
        public void verifyAgainst(OverlayMembershipSnapshot membership) throws GeneralSecurityException {
            OverlayKeyAgreement.validateMembership(projectId, membershipRevision, originNodeId, destinationNodeId,
                    membership);
        ...
        private byte[] encodeUnsigned() {
            return OverlayKeyAgreement.encodeUnsigned(MAGIC, INIT, projectId, originNodeId, destinationNodeId, sessionId,
    ...
    public static final class ResponseRecord {
        ...
        private ResponseRecord(UUID projectId, String originNodeId, String destinationNodeId, UUID sessionId,
                long membershipRevision, byte[] responseNonce, byte[] ephemeralPublicKey, byte[] transcriptHash,
                byte[] signature) {
            this.projectId = Objects.requireNonNull(projectId, "project ID");
            OverlayCodecSupport.requireNodeId(originNodeId);
            OverlayCodecSupport.requireNodeId(destinationNodeId);
        ...
        private static ResponseRecord unsigned(InitRecord init, byte[] responseNonce, byte[] ephemeralPublicKey,
                byte[] transcriptHash) {
            return new ResponseRecord(init.projectId, init.originNodeId, init.destinationNodeId, init.sessionId,
        ...
        private static ResponseRecord signed(ResponseRecord unsigned, byte[] signature) {
            return new ResponseRecord(unsigned.projectId, unsigned.originNodeId, unsigned.destinationNodeId,
        ...
        public static ResponseRecord decode(byte[] bytes) throws IOException {
            ...
            try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
                ...
                OverlayCodecSupport.requireNoTrailing(input);
                ResponseRecord result = new ResponseRecord(project, origin, destination, session, revision, nonce,
        ...
        private byte[] encodeUnsigned() {
            return OverlayKeyAgreement.encodeUnsigned(MAGIC, RESPONSE, projectId, originNodeId, destinationNodeId, sessionId,
    ...
    private static byte[] encodeUnsigned(int magic, int kind, UUID projectId, String originNodeId,
            String destinationNodeId, UUID sessionId, long membershipRevision, byte[] nonce, byte[] ephemeral,
    ...
    public static final class E2eSession implements AutoCloseable {
        ...
        private E2eSession(UUID projectId, UUID sessionId, String sendOriginNodeId, String sendDestinationNodeId,
                String receiveOriginNodeId, String receiveDestinationNodeId, byte[] sendKey, byte[] sendNoncePrefix,
        ...
        private OverlayEnvelope seal(UUID messageId, long sequence, byte[] plaintext)
            ...
            return OverlayEnvelope.sealed(projectId, messageId, sendOriginNodeId, sendDestinationNodeId, sessionId,
                    sequence, OverlayCodecSupport.aeadEncrypt(sendKey, nonce(sendNoncePrefix, sequence), aad, plaintext));
        }
        ...
        public synchronized byte[] decrypt(OverlayEnvelope envelope) throws GeneralSecurityException {
            ...
            if (!projectId.equals(envelope.projectId()) || !receiveOriginNodeId.equals(envelope.originNodeId())
            ...
            byte[] plaintext = OverlayCodecSupport.aeadDecrypt(receiveKey, nonce(receiveNoncePrefix, envelope.sequence()),
