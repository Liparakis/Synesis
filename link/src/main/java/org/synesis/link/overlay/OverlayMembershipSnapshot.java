...
public final class OverlayMembershipSnapshot {
  ...
  private OverlayMembershipSnapshot(UUID projectId, String authorityNodeId, long revision,
      Instant issuedAt, Instant expiresAt, List<Member> members, byte[] unsignedBytes,
      byte[] signature) {
  ...
  public static OverlayMembershipSnapshot create(UUID projectId, long revision, Instant issuedAt,
      Instant expiresAt, List<Member> members, NodeIdentity authority)
    ...
    byte[] unsigned = encodeUnsigned(projectId, authority.nodeId(), revision, issuedAt, expiresAt,
        sorted);
    return new OverlayMembershipSnapshot(projectId, authority.nodeId(), revision, issuedAt,
        expiresAt, sorted,
  ...
  public static OverlayMembershipSnapshot decode(byte[] bytes) throws IOException {
    ...
    try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      String authority = OverlayCodecSupport.nodeId(
      ...
      for (int index = 0; index < count; index++) {
        String nodeId = OverlayCodecSupport.nodeId(
            input.readNBytes(OverlayCodecSupport.NODE_ID_BYTES));
      ...
      OverlayMembershipSnapshot result = new OverlayMembershipSnapshot(project, authority, revision,
          issued,
      ...
      return result;
    } catch (java.io.EOFException | IllegalArgumentException |
  ...
  private static byte[] encodeUnsigned(UUID projectId, String authorityNodeId, long revision,
      Instant issuedAt,
  ...
  public boolean verify() {
    try {
      ...
      return authority != null && authority.status() == STATUS_ACTIVE
          && OverlayCodecSupport.verifyEd25519(authority.publicKeyEncoded(), unsignedBytes,
          signature);
