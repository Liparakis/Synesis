 ...
 * It has no durable mailbox and no API that accepts arbitrary IP traffic. A Netty or other
 * event-loop server can bind its connection callbacks to this class without changing the
 * authorization and bound checks.
 */
public final class OverlayRelay {
   ...
   */
  public Registration register(NodeIdentity identity, Connection connection)
  ...
  /**
   * Registers a node after an external transport has verified its durable identity proof.
   *
   ...
   * @throws GeneralSecurityException if the node is not authorized
   */
  public synchronized Registration registerAuthenticated(String authenticatedNodeId,
  ...
  private CompletionStage<Void> forward(String sourceNodeId, OverlayForwardingFrame frame,
      Instant now) {
    try {
