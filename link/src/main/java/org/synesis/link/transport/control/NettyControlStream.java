...
public final class NettyControlStream extends SimpleChannelInboundHandler<ByteBuf> implements
  ...
  /**
   * Maximum graceful-close wait before the control stream is forced closed.
   */
  public static final int MAX_CLOSE_MILLIS = 2_000;
  ...
  /**
   * Creates the bounded Netty control-stream owner for an authenticated Link session.
   *
   * @param session               authenticated Link session
   * @param established           completion signaled when the control stream becomes ready
   * @param claim                 callback claiming the session's control stream
   * @param livenessConfiguration liveness settings for the heartbeat loop
   * @return configured control-stream owner
   */
  public static NettyControlStream create(PeerSession session,
      CompletableFuture<PeerSession> established,
      Runnable claim,
      LivenessConfiguration livenessConfiguration) {
  ...
  private void validateSessionPayload(byte[] payload, int expectedLength)
  ...
  public CompletionStage<DemoWorkResult> request(DemoWorkRequest request) {
    if (!isReady() || terminal.isDone()) {
      return CompletableFuture.failedFuture(
          new IllegalStateException("demo work requires a live control stream"));
    }
  ...
  public LivenessMetrics livenessMetrics() {
    return liveness == null ? new LivenessMetrics(0, 0, 0, 0, 0, Duration.ZERO, Duration.ZERO, 0, 0,
        0, 0, 0)
        : liveness.metrics();
  }
  ...
  public void channelInactive(ChannelHandlerContext context) {
    ...
    if (!established.isDone()) {
      established.completeExceptionally(
          new HandshakeException(HandshakeFailureCode.MALFORMED_HANDSHAKE));
    }
    finishTerminal();
  }
