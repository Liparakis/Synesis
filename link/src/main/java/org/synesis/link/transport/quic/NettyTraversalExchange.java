 ...
 * sends its signed offer to advertised candidates, and a responder answers to
 * the source endpoint observed on the same UDP channel. Non-traversal datagrams
 * are forwarded to the existing QUIC codec. The class owns no socket.
 */
public final class NettyTraversalExchange extends SimpleChannelInboundHandler<DatagramPacket>
     ...
     * @param responder optional responder callback; {@code null} makes this an initiator-only exchange
     */
    ...
    public java.util.concurrent.CompletionStage<TraversalAnswer> sendOffer(TraversalOffer offer,
        ...
        if (closed.get() || !channel.isOpen()) {
            return CompletableFuture.failedFuture(new IllegalStateException("traversal channel is closed"));
        }
        if (candidates.isEmpty() || candidates.size() > MAX_TARGETS) {
            return CompletableFuture.failedFuture(new IllegalArgumentException("invalid traversal target bound"));
        }
